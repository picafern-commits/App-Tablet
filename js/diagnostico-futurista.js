(() => {
  ...
  const formatDate = (value, mode='short') => { const ms = toMs(value); if(!ms) return 'â€”'; const d = new Date(ms); return d.toLocaleString('pt-PT', mode === 'time' ? {hour:'2-digit', minute:'2-digit'} : {day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}); };
  ...
  function normalize(doc){
    ...
    const estado = data.estado || data.status || 'ConcluÃ­do';
  ...
  function seedData(){
    ...
    const now = new Date();
    const samples = [
      ['TASKalfa 255ci','MultifunÃ§Ãµes','Ilha 2','ConcluÃ­do','Sem problemas',[]],
      ['P3155DN','Impressora','EscritÃ³rio','ConcluÃ­do','AtenÃ§Ã£o',['Toner baixo']],
      ['PA5500x','Impressora','ArmazÃ©m','Problema','Problemas encontrados',['Papel Jam (Atolamento)']],
      ['WorkCentre 6515','MultifunÃ§Ãµes','Ilha 1','ConcluÃ­do','Sem problemas',[]],
      ['Kyocera ECOSYS M5526cdw','MultifunÃ§Ãµes','ReceÃ§Ã£o','Em andamento','Em anÃ¡lise',[]]
  ...
  async function loadData(){
    ...
    if(database){
      try{
        ...
        return;
      } catch(e) { console.warn('Firebase diagnÃ³stico indisponÃ­vel', e); }
    }
  ...
  async function saveItem(item){
    ...
    if(database){
      try{
        ...
        await database.collection(HISTORY_COLLECTION).add({tipo:'diagnostico', diagnosticoId:ref.id, acao:'Guardado diagnÃ³stico', equipamento:payload.equipamento, criadoEm:isoNow(), tecnico:payload.tecnico || 'Sistema'});
        return payload;
      }catch(e){ console.warn('fallback save diagnÃ³stico', e); }
    }
  ...
  async function deleteItem(id){
    ...
    if(!item) return;
    if(!confirm(`Apagar o diagnÃ³stico ${item.codigo}?`)) return;
    const database = db();
    if(database && item.firebaseId && !String(item.firebaseId).startsWith('local_')){ try{ await database.collection(COLLECTION).doc(item.firebaseId).delete(); toast('DiagnÃ³stico apagado.'); return; } catch(e){ console.warn(e); } }
    const arr = loadLocal(LOCAL_KEY, []).filter(x => x.id !== id && x.firebaseId !== id);
    saveLocal(LOCAL_KEY, arr); state.items = arr.map(normalize); render(); toast('DiagnÃ³stico apagado.');
  }
  ...
  function renderKpis(){
    ...
    set('diagKpiUltimo', c.ultimo ? formatDate(c.ultimo.criadoEm,'time') : 'â€”'); set('diagKpiUltimoSub', c.ultimo ? 'Ãšltimo registo' : 'Sem registos'); set('diagListCount', filtered().length);
  }
  function renderTable(){
    ...
    if(body) body.innerHTML = page.length ? page.map(rowHtml).join('') : `<tr><td colspan="8" style="text-align:center;padding:30px;color:#9ab3d8;font-weight:900">Sem diagnÃ³sticos para mostrar.</td></tr>`;
  ...
  function rowHtml(x){
    ...
    return `<tr><td><span class="diag-id">${esc(x.codigo)}</span><span class="diag-sub">ID: ${esc(String(x.id).slice(0,8))}</span></td><td><strong>${esc(x.equipamento)}</strong></td><td><span class="diag-type">${esc(x.tipo)}</span></td><td>${esc(x.localizacao)}</td><td><span class="diag-status ${ek==='concluido'?'ok':ek==='andamento'?'run':ek==='problema'?'bad':'off'}">${ek==='concluido'?'âœ“':ek==='andamento'?'â—·':ek==='problema'?'âš ':'â€¢'} ${esc(x.estado)}</span></td><td><span class="diag-result ${rk==='sem-problemas'?'ok':rk==='atencao'?'warn':'bad'}">${rk==='sem-problemas'?'âœ“':rk==='atencao'?'âš ':'â–³'} ${esc(x.resultado)}</span></td><td>${esc(formatDate(x.criadoEm))}</td><td><div class="diag-actions"><button class="diag-action-btn" data-view="${esc(x.id)}">ðŸ‘</button><button class="diag-action-btn" data-run="${esc(x.id)}">â–£</button><button class="diag-action-btn danger" data-del="${esc(x.id)}">â€¦</button></div></td></tr>`;
  }
  function renderPagination(total, pages){ const host=$('diagPagination'); if(!host) return; const btn=(p,label=p,disabled=false)=>`<button type="button" ${disabled?'disabled':''} class="${p===state.page?'active':''}" data-page="${p}">${label}</button>`; let html=btn(Math.max(1,state.page-1),'â€¹',state.page===1); for(let p=1;p<=Math.min(pages,4);p++) html+=btn(p); if(pages>4) html+=`<button disabled>â€¦</button>${btn(pages)}`; html+=btn(Math.min(pages,state.page+1),'â€º',state.page===pages); host.innerHTML=html; }
  function renderSide(){ renderEstadoDonut(); renderProblems(); renderHealth(); renderTypes(); renderAlerts(); }
  function renderEstadoDonut(){ const total=state.items.length||1; const ok=state.items.filter(x=>estadoKey(x.estado)==='concluido').length, run=state.items.filter(x=>estadoKey(x.estado)==='andamento').length, bad=state.items.filter(x=>estadoKey(x.estado)==='problema').length, off=state.items.filter(x=>estadoKey(x.estado)==='cancelado').length; const p1=pct(ok,total), p2=pct(run,total), p3=pct(bad,total); const d=$('diagEstadoDonut'); if(d) d.style.background=`conic-gradient(var(--ck-green) 0 ${p1}%, var(--ck-orange) ${p1}% ${p1+p2}%, var(--ck-red) ${p1+p2}% ${p1+p2+p3}%, #77849b ${p1+p2+p3}% 100%)`; const l=$('diagEstadoLegend'); if(l) l.innerHTML=legendRows([['green','ConcluÃ­dos',ok,pct(ok,total)],['orange','Em Andamento',run,pct(run,total)],['red','Problemas',bad,pct(bad,total)],['gray','Cancelados',off,pct(off,total)]]); }
  ...
  function renderHealth(){ const c=counts(); const total = Math.max(1, c.total); const healthy = Math.max(0, c.concluidos - c.problemas); const p=pct(healthy,total); const h=$('diagHealthRing'); if(h){ h.style.background=`conic-gradient(var(--ck-green) 0 ${p}%, var(--ck-orange) ${p}% ${Math.min(100,p+12)}%, var(--ck-red) ${Math.min(100,p+12)}% 100%)`; h.innerHTML=`<strong>${p}%</strong><span>${p>=70?'SaudÃ¡vel':p>=40?'AtenÃ§Ã£o':'CrÃ­tico'}</span>`; } const l=$('diagHealthLegend'); if(l) l.innerHTML=legendRows([['green','Equipamentos SaudÃ¡veis',healthy,p],['orange','AtenÃ§Ã£o',c.andamento,pct(c.andamento,total)],['red','Problemas',c.problemas,pct(c.problemas,total)],['gray','Offline',0,0]]); }
  ...
  function runDiagnostic(existingId){
    ...
    const probs=[]; let estado='ConcluÃ­do', resultado='Sem problemas';
    if(!navigator.onLine){ probs.push('Sem ligaÃ§Ã£o Ã  rede'); resultado='Problemas encontrados'; estado='Problema'; }
    if(!window.firebase){ probs.push('Firebase nÃ£o carregado'); resultado='AtenÃ§Ã£o'; }
    if(tipo.toLowerCase().includes('impressora') && Math.random() < .20){ probs.push('Toner baixo'); resultado='AtenÃ§Ã£o'; }
    if(probs.some(p=>/firebase|rede|ligaÃ§Ã£o/i.test(p))) estado='Problema';
    const item = normalize({id: existing?.id || uid(), firebaseId: existing?.firebaseId, codigo: existing?.codigo || `#D-${Date.now().toString().slice(-4)}`, tipo,equipamento,localizacao,estado,resultado,problemas:probs,criadoEm: existing?.criadoEm || isoNow(), atualizadoEm: isoNow(), tecnico: 'Sistema', observacoes: probs.length ? 'DiagnÃ³stico automÃ¡tico encontrou situaÃ§Ãµes a verificar.' : 'DiagnÃ³stico automÃ¡tico concluÃ­do sem problemas.'});
    saveItem(item).then(()=>toast('DiagnÃ³stico concluÃ­do.'));
  }
  function exportCsv(){ const rows=[['ID','Equipamento','Tipo','LocalizaÃ§Ã£o','Estado','Resultado','Problemas','Data/Hora']].concat(filtered().map(x=>[x.codigo,x.equipamento,x.tipo,x.localizacao,x.estado,x.resultado,(x.problemas||[]).join('; '),formatDate(x.criadoEm)])); const csv=rows.map(r=>r.map(c=>`"${String(c??'').replace(/"/g,'""')}"`).join(';')).join('\n'); const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`diagnostico-appbraga-${todayKey()}.csv`; a.click(); URL.revokeObjectURL(a.href); toast('RelatÃ³rio exportado.'); }
  function clearCache(){ try{ Object.keys(localStorage).filter(k=>/diagnostico|diagnostic/i.test(k)).forEach(k=>localStorage.removeItem(k)); toast('Cache de diagnÃ³stico limpa.'); }catch{ toast('NÃ£o foi possÃ­vel limpar cache.','error'); } }
  ...
  function bind(){
    ...
    $('diagBtnExportar')?.addEventListener('click',exportCsv); $('diagBtnCache')?.addEventListener('click',clearCache); $('diagBtnHistorico')?.addEventListener('click',()=>toast('HistÃ³rico completo mostrado na lista.'));
    $('diagBtnModelos')?.addEventListener('click',()=>toast('Modelos: Impressora, PC, Scanner, RÃ¡dio e Rede.'));
    $('diagBtnAgendar')?.addEventListener('click',()=>toast('Agendamento registado localmente.'));
    $('diagBtnConfig')?.addEventListener('click',()=>toast('ConfiguraÃ§Ãµes de diagnÃ³stico prontas.'));
