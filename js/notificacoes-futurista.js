(() => {
  ...
  const formatDate = (value, mode='short') => { const ms = toMs(value); if(!ms) return '—'; const d = new Date(ms); return d.toLocaleString('pt-PT', mode === 'date' ? {day:'2-digit',month:'2-digit',year:'numeric'} : {day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}); };
  ...
  function normalize(doc){
    ...
    return {
      ...
      titulo: data.titulo || data.title || data.assunto || 'Sem título',
    ...
    if(database){
      try{
        COLLECTIONS.forEach(col => {
          ...
          }, err => console.warn('notificações snapshot', col, err));
        ...
        HISTORY_COLLECTIONS.forEach(col => {
          ...
          }, err => console.warn('notificações historico snapshot', col, err));
          state.unsubs.push(unsub);
        });
        ...
        return;
      }catch(e){ console.warn('Firebase notificações indisponível', e); }
    }
  ...
  async function saveItem(item){
    ...
    if(database){
      try{
        ...
        await database.collection(HISTORY_COLLECTIONS[0]).add({notificacaoId:ref.id,titulo:payload.titulo,tipo:payload.tipo,acao:'Notificação guardada',estado:payload.estado,criadoEm:isoNow(),autor:payload.autor});
        return payload;
  ...
  async function deleteItem(id){
    ...
    if(!confirm(`Apagar a notificação "${item.titulo}"?`)) return;
    const database = db();
    if(database && item.firebaseId && !String(item.firebaseId).startsWith('local_')){
      try{ await database.collection(item._col || COLLECTIONS[0]).doc(item.firebaseId).delete(); toast('Notificação apagada.'); return; }catch(e){ console.warn(e); }
    }
    const arr = loadLocal(LOCAL_KEY, []).filter(x => x.id !== id && x.firebaseId !== id);
    saveLocal(LOCAL_KEY, arr); state.items = arr.map(normalize); render(); toast('Notificação apagada.');
  }
  ...
  function renderKpis(){
    ...
    setText('notKpiPendentes', c.pendente); setText('notKpiFalhas', c.falha); setText('notKpiAlvos', c.activeDevices || '—');
    const last = state.items.slice().sort((a,b)=>toMs(b.enviadoEm)-toMs(a.enviadoEm))[0];
    setText('notKpiUltima', last ? formatLast(last.enviadoEm) : '—'); setText('notKpiUltimaSub', last ? ago(last.enviadoEm) : 'Sem registos');
  }
  function formatLast(value){ const ms=toMs(value); if(!ms) return '—'; const d=new Date(ms); return d.toDateString()===new Date().toDateString() ? `Hoje, ${d.toLocaleTimeString('pt-PT',{hour:'2-digit',minute:'2-digit'})}` : formatDate(value,'date'); }
  function ago(value){ const diff = Date.now()-toMs(value); if(!toMs(value)) return 'Sem registos'; if(diff<60000) return 'Agora'; if(diff<3600000) return `${Math.round(diff/60000)} min atrás`; if(diff<86400000) return `${Math.round(diff/3600000)} h atrás`; return `${Math.round(diff/86400000)} dias atrás`; }
  ...
  function renderTable(){
    ...
    const body = $('notTableBody'); if(body){
      body.innerHTML = slice.length ? slice.map(rowHtml).join('') : `<tr><td colspan="8" style="text-align:center;color:#9ab3d8;padding:26px">Sem notificações para mostrar.</td></tr>`;
    }
  ...
  function rowHtml(item){
    ...
    return `<tr><td><strong>${esc(item.codigo)}</strong></td><td class="not-title-cell"><strong>${esc(item.titulo)}</strong><small>${esc(item.mensagem || '—')}</small></td><td><span class="ck-badge blue">${esc(item.tipo)}</span></td><td>${esc(item.destinatarios)}</td><td><span class="ck-badge ${priClass}">${esc(pk)}</span></td><td><span class="ck-badge ${estClass}">${esc(item.estado)}</span></td><td>${esc(formatDate(item.enviadoEm || item.criadoEm))}</td><td><div class="not-actions"><button class="ck-icon-btn" type="button" data-view="${esc(item.id)}" title="Ver">👁</button><button class="ck-icon-btn" type="button" data-resend="${esc(item.id)}" title="Reenviar">📤</button><button class="ck-icon-btn red" type="button" data-delete="${esc(item.id)}" title="Apagar">🗑</button></div></td></tr>`;
  }
  function renderPagination(pages){
    const el = $('notPagination'); if(!el) return; let html='';
    html += `<button type="button" ${state.page<=1?'disabled':''} data-page="${state.page-1}">«</button>`;
  ...
  function renderCritical(){
    ...
    el.innerHTML = critical.length ? critical.map(x=>`<div class="not-feed-item"><span class="not-feed-icon">${estadoKey(x.estado)==='falha'?'×':'!'}</span><div><strong>${esc(x.titulo)}</strong><small>${esc(x.mensagem || x.tipo)}</small></div><time>${esc(formatLast(x.enviadoEm || x.criadoEm))}</time></div>`).join('') : '<p style="color:#9ab3d8;margin:10px 0">Sem alertas críticos.</p>';
  }

  function getFormPayload(){
    const titulo = text($('notTitulo')?.value); const mensagem = text($('notMensagem')?.value);
    if(!titulo) throw new Error('Escreve um título para a notificação.');
    if(!mensagem) throw new Error('Escreve uma mensagem para a notificação.');
    const agendar = $('notAgendar')?.value || '';
    const estado = agendar && toMs(agendar) > Date.now() ? 'Agendada' : 'Enviada';
    return {titulo,mensagem,tipo:$('notTipo')?.value || 'Sistema',destinatarios:$('notDestinatarios')?.value || 'Todos',prioridade:$('notPrioridade')?.value || 'Média',estado,agendar,enviadoEm: estado==='Enviada'?isoNow():agendar,criadoEm:isoNow(),autor:'AppBraga'};
  }
  async function submitNotification(){
    try{ const payload = getFormPayload(); await saveItem(payload); clearForm(); toast(payload.estado === 'Agendada' ? 'Notificação agendada.' : 'Notificação guardada/enviada.'); }
    catch(e){ toast(e.message || 'Erro ao guardar notificação.', 'error'); }
  }
  ...
  function exportCsv(){
    const rows = [['ID','Título','Mensagem','Tipo','Destinatários','Prioridade','Estado','Data/Hora']].concat(filtered().map(x=>[x.codigo,x.titulo,x.mensagem,x.tipo,x.destinatarios,x.prioridade,x.estado,formatDate(x.enviadoEm || x.criadoEm)]));
  ...
  async function clearAll(){
    if(!state.items.length){ toast('Não existem notificações para limpar.'); return; }
    const phrase = prompt(`Para limpar todas as ${state.items.length} notificações escreve: LIMPAR NOTIFICACOES`);
    if(phrase !== 'LIMPAR NOTIFICACOES'){ toast('Operação cancelada.'); return; }
    const database = db();
    if(database){
      try{
        ...
        await batch.commit(); toast('Notificações limpas.'); return;
      }catch(e){ console.warn(e); }
    }
    saveLocal(LOCAL_KEY, []); state.items=[]; render(); toast('Notificações limpas.');
  }
  async function resend(id){ const item = state.items.find(x=>x.id===id || x.firebaseId===id); if(!item) return; await saveItem({...item, estado:'Enviada', enviadoEm:isoNow(), criadoEm:item.criadoEm || isoNow()}); toast('Notificação reenviada/guardada.'); }
  ...
  function bind(){
    ...
    $('notBtnExportar')?.addEventListener('click', exportCsv); $('notBtnLimparTodas')?.addEventListener('click', clearAll); $('notBtnHistorico')?.addEventListener('click', () => openModal('Histórico completo', 'Últimas ações registadas.', historyHtml()));
    $('notBtnAgendar')?.addEventListener('click', ()=>$('notAgendar')?.focus()); $('notBtnDestinatarios')?.addEventListener('click', () => openModal('Destinatários', 'Dispositivos registados para notificações.', devicesHtml()));
    $('notBtnConfig')?.addEventListener('click', () => openModal('Configurações', 'Estado atual do sistema de notificações.', `<p>Firebase: ${db()?'ativo':'fallback local'}<br>Coleções: ${COLLECTIONS.join(', ')}<br>Versão: ${VERSION}</p>`));
    $('notBtnAlertas')?.addEventListener('click', () => openModal('Alertas críticos', 'Notificações de alta prioridade ou falhas.', $('notCriticalList')?.innerHTML || 'Sem alertas.'));
  ...
  function historyHtml(){ const items = state.history.slice(0,30); return items.length ? `<div class="not-feed">${items.map(x=>`<div class="not-feed-item"><span class="not-feed-icon">▣</span><div><strong>${esc(x.titulo || x.acao || 'Registo')}</strong><small>${esc(x.tipo || x.estado || 'Histórico')}</small></div><time>${esc(formatDate(x.criadoEm || x.enviadoEm))}</time></div>`).join('')}</div>` : '<p>Sem histórico registado.</p>'; }
