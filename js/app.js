...
async function disponivel() {
  ...
  try {
    ...
    // v1.58.19: a etiqueta da página Etiquetas deve ficar guardada
...
function maybeNotifyCriticalSupply(ip, info, previousInfo = null) {
  // v1.58.19: nao enviar notificacoes cloud/local a partir das paginas.
...
async function maybeNotifyTonerReplacement(ip, previousInfo, nextInfo) {
  // v1.58.19: reposicao de toner tambem fica centralizada nas Cloud Functions.
  return;
...
async function verificarAlertasNotificacoesApp(force = false) {
  // v1.58.19: alertas automáticos deixaram de correr no cliente.
...
async function notificarAlteracaoRealtimeApp(collectionKey, snapshot) {
  // v1.58.19: não criar notificações automáticas a partir de listeners de página.
...
function iniciarMonitorNotificacoesApp() {
  clearInterval(appNotificationTimer);
  // v1.58.19: sem monitor automático no cliente para evitar loops no Dashboard.
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // v1.58.19: no Electron/GitHub a leitura SNMP tem de correr como serviço global,
...
async function syncPrinterInfoToFirebase(ip, info) {
  ...
  // v1.58.19: o Dashboard/Electron estava a escrever novamente os mesmos valores
...
function bindPrintersFirebaseRealtime() {
  ...
  // v1.58.19: a leitura Firebase das impressoras tem de estar ativa em todas as páginas.
...
async function guardarEtiquetaPartilhada(extra = {}) {
  ...
  try {
    ...
    // v1.58.19: evitar duplicados. Ao adicionar toner, a etiqueta é guardada
