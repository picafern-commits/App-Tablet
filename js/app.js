...
const APP_ROLE_LABELS = {
  admin: "Admin",
  tecnico: "TÃƒÆ’Ã‚Â©cnico",
  armazem: "ArmazÃƒÆ’Ã‚Â©m",
  consulta: "Consulta"
};
...
async function carregarPermissoesApp() {
  try {
    ...
    console.warn("Erro ao carregar permissÃƒÆ’Ã‚Âµes:", error);
...
const impressorasData = [
  ...
  { modelo: "Kyocera P3155dn", serie: "R4B1293183", armazem: "Braga", localizacao: "BalcÃƒÆ’Ã‚Â£o 01", ip: "192.168.10.184" },
  { modelo: "Kyocera P3155dn", serie: "R4B1293184", armazem: "Braga", localizacao: "BalcÃƒÆ’Ã‚Â£o 02", ip: "192.168.10.183" },
  ...
  { modelo: "Kyocera P3155dn", serie: "R4B1395261", armazem: "Braga", localizacao: "DevoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes", ip: "192.168.10.187" },
...
/* =========================
   IMPRESSORAS / MANUTENÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O
========================= */
...
function badgeEstado(estado) {
  ...
  if (estado === "Em reparaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o") return `<span class="badge reparacao">Em reparaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</span>`;
...
function preencherLocaisManutencao() {
  ...
  if (selectLoc) {
    selectLoc.innerHTML = `
      <option value="">Selecionar localizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</option>
  ...
  if (selectSerie) {
    selectSerie.innerHTML = `
      <option value="">Selecionar nÃƒâ€šÃ‚Âº sÃƒÆ’Ã‚Â©rie</option>
      ${impressorasData.map(item => `
...
async function reiniciarContadorTonerConfig() {
  if (!confirm("Reiniciar o contador dos toners? O prÃƒÆ’Ã‚Â³ximo toner criado vai ser TON-0001.")) return;
  ...
  try {
    ...
    mostrarMensagem("Contador reiniciado. O prÃƒÆ’Ã‚Â³ximo toner serÃƒÆ’Ã‚Â¡ TON-0001.");
  } catch (error) {
    console.error(error);
...
async function disponivel() {
  ...
  if (!equipamento || !cor) {
    mostrarMensagem("Sistema de adicionar toner nÃƒÂ£o encontrou o formulÃƒÂ¡rio.", "erro");
    return;
  }

  const database = getDbAppBraga();
  if (!database || !database.collection) {
    mostrarMensagem("Firebase indisponÃƒÂ­vel. Liga a internet e volta a tentar.", "erro");
    return;
  }
  ...
  try {
    ...
    const payload = sanitizeFirestorePayloadAppBraga({
      idInterno: id,
      equipamento: eq,
      localizacao: loc || "Sem LocalizaÃƒÂ§ÃƒÂ£o",
      cor: corValue,
    ...
    try {
      ...
      console.warn("Toner guardado, mas a etiqueta automÃƒÂ¡tica falhou:", etiquetaError);
    }
    ...
    mostrarMensagem(
      etiquetaGerada
        ...
        : "Toner adicionado ao stock. A etiqueta ficou disponÃƒÂ­vel para gerar manualmente.",
      "sucesso"
    );
  } catch (error) {
...
function atualizarContadoresManutencao() {
  ...
  setText("countManutReparacao", manutencoesGlobal.filter(i => i.estado === "Em reparaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o").length);
...
function getTopLocalizacoesHistorico(limit = 3) {
  ...
  historicoGlobal.forEach(item => {
    const key = String(item.localizacao || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o");
...
function renderDashboardResumoInteligente() {
  ...
  const critLabel = buckets.critical > 0 ? "AÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o imediata" : "Sem crÃƒÆ’Ã‚Â­ticos";
  ...
  host.innerHTML = `
    <div class="summary-grid">
      <div class="summary-card">
        ...
        <div class="meta-line">${critLabel} Ãƒâ€šÃ‚Â· toner a 0%</div>
      </div>
      <div class="summary-card">
        <h4>AtenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</h4>
        <div class="summary-value">${buckets.warning}</div>
        <div class="meta-line">${warnLabel} Ãƒâ€šÃ‚Â· sem avisos intermÃƒÆ’Ã‚Â©dios de toner</div>
      </div>
      <div class="summary-card">
        <h4>Top LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes</h4>
        <ul class="summary-list">${topLocs.length ? topLocs.map(([k,v]) => `<li>${k} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${v}</li>`).join("") : "<li>Sem dados ainda</li>"}</ul>
      </div>
      <div class="summary-card">
        <h4>ÃƒÆ’Ã…Â¡ltimos Movimentos</h4>
        <ul class="summary-list">${ultimos.length ? ultimos.map(item => `<li>${item.equipamento || "-"} Ãƒâ€šÃ‚Â· ${item.cor || "-"} Ãƒâ€šÃ‚Â· ${item.localizacao || "-"}</li>`).join("") : "<li>Sem histÃƒÆ’Ã‚Â³rico ainda</li>"}</ul>
      </div>
    </div>`;
}
...
function renderDashboardCards(items) {
  ...
  if (!criticas.length) {
    lista.innerHTML = `<div class="panel empty-state"><h3>Sem toners abaixo de 25%</h3><p>As impressoras com toner a 25% ou menos vÃƒÆ’Ã‚Â£o aparecer aqui.</p></div>`;
    return;
  }
  ...
  lista.innerHTML = criticas.map(({ item, info, criticalColors, monoCritical, residue }, index) => {
    ...
    const residueHtml = residue ? gerarHTMLBarraToner(residue.percent, residue.label || "ResÃƒÆ’Ã‚Â­duo", "waste") : "";
    ...
    return `
      <div class="dashboard-card dashboard-critical-card">
        ...
        <div class="meta-line">SÃƒÆ’Ã‚Â©rie: <span class="meta-value">${item.serie}</span></div>
...
function renderStockCards(items) {
  ...
  lista.innerHTML = items.map(t => `
    <div class="stock-card">
      ...
      <div class="meta-line">LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: <span class="meta-value">${t.localizacao}</span></div>
      ...
      <div class="meta-line">CÃƒÆ’Ã‚Â³digo etiqueta: <span class="meta-value">${t.codigoEtiqueta || "-"}</span></div>
...
function renderHistoricoCards(items) {
  ...
  if (!items.length) {
    lista.innerHTML = `<div class="panel empty-state"><h3>Sem histÃƒÆ’Ã‚Â³rico</h3><p>Os toners usados vÃƒÆ’Ã‚Â£o aparecer aqui.</p></div>`;
    return;
  }
  ...
  lista.innerHTML = items.map(t => `
    <div class="history-card">
      ...
      <div class="meta-line">Equipamento: <span class="meta-value">${t.equipamento}</span></div>
      <div class="meta-line">Cor: <span class="meta-value">${t.cor || "-"}</span></div>
      <div class="meta-line">LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: <span class="meta-value">${t.localizacao || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o"}</span></div>
      <div class="meta-line">Lote: <span class="meta-value">${t.lote || "-"}</span></div>
      <div class="meta-line">SDS Ref: <span class="meta-value">${t.sdsRef || "-"}</span></div>
      <div class="meta-line">CÃƒÆ’Ã‚Â³digo etiqueta: <span class="meta-value">${t.codigoEtiqueta || "-"}</span></div>
      <div class="meta-line">Data Scan: <span class="meta-value">${t.data || "Sem Data"}</span></div>
...
async function usar(id) {
  try {
    ...
    if (!snap.exists) {
      mostrarMensagem("Toner nÃƒÆ’Ã‚Â£o encontrado.", "erro");
      return;
    }
    ...
    mostrarMensagem("Toner movido para histÃƒÆ’Ã‚Â³rico.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao mover para histÃƒÆ’Ã‚Â³rico.", "erro");
  }
}
...
async function usarPorCodigoEtiquetaToner(codigoOuPayload) {
  ...
  try {
    ...
    if (!id) {
      mostrarMensagem("CÃƒÆ’Ã‚Â³digo de toner nÃƒÆ’Ã‚Â£o encontrado em stock.", "erro");
      return true;
    }

    await usar(id);
    mostrarMensagem(`Toner ${codigo || rawUpper} passado para histÃƒÆ’Ã‚Â³rico.`);
    return true;
  } catch (error) {
    console.error(error);
...
async function apagar(id) {
  try {
    ...
    mostrarMensagem("HistÃƒÆ’Ã‚Â³rico apagado.");
  } catch (error) {
    console.error(error);
...
function renderPCCards(items) {
  ...
  lista.innerHTML = items.map((d, index) => {
    const htmlPassos = (d.passos || []).map(p => `
      <div class="meta-line">${p.feito ? "ÃƒÂ¢Ã…â€œÃ¢â‚¬Â" : "ÃƒÂ¢Ã‚ÂÃ…â€™"} <span class="meta-value">${p.passo}</span></div>
    `).join("");
...
async function guardarPC() {
  ...
  if (!nome) {
    mostrarMensagem("Nome obrigatÃƒÆ’Ã‚Â³rio.", "erro");
    return;
  }
...
async function guardarManutencao() {
  ...
  if (!tecnico || !armazem || !localizacao || !modelo || !serie || !ip || !motivo || !dataPedido) {
    mostrarMensagem("Preenche os campos obrigatÃƒÆ’Ã‚Â³rios da manutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
    return;
  }
  ...
  try {
    await db.collection("manutencoes").add({
      ...
      motivo,
      dataPedido,
      dataResolucao: dataResolucao || "Sem resoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o",
      created: new Date()
    });

    limparFormularioManutencao();
    mostrarMensagem("ManutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o guardada com sucesso.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao guardar manutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
  }
}
...
function renderManutencoes(items) {
  ...
  if (!items.length) {
    lista.innerHTML = `
      <div class="panel empty-state">
        <h3>Sem pedidos de manutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</h3>
        <p>Os pedidos vÃƒÆ’Ã‚Â£o aparecer aqui.</p>
      </div>
    `;
    return;
  }
  ...
  lista.innerHTML = items.map(item => `
    <div class="pc-card manut-card">
      ...
      <div class="meta-line">TÃƒÆ’Ã‚Â©cnico: <span class="meta-value">${item.tecnico}</span></div>
      <div class="meta-line">ArmazÃƒÆ’Ã‚Â©m: <span class="meta-value">${item.armazem}</span></div>
      <div class="meta-line">LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: <span class="meta-value">${item.localizacao}</span></div>
      ...
      <div class="meta-line">ResoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: <span class="meta-value">${item.dataResolucao || "Sem resoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o"}</span></div>
...
async function marcarResolvido(id) {
  try {
    ...
    mostrarMensagem("ManutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o marcada como resolvida.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao atualizar manutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
  }
}
...
async function apagarManutencao(id) {
  try {
    ...
    mostrarMensagem("Registo de manutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o apagado.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao apagar manutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
  }
}
...
function corBarraToner(percentagem, cor = "black") {
  ...
  // ResÃƒÆ’Ã‚Â­duo ÃƒÆ’Ã‚Â© ao contrÃƒÆ’Ã‚Â¡rio: quanto maior pior.
  if (cor === "waste") {
  ...
  if (value <= 10) return "#dc2626";  // vermelho crÃƒÆ’Ã‚Â­tico
  if (value <= 25) return "#f97316";  // laranja baixo
  if (value <= 50) return "#eab308";  // amarelo mÃƒÆ’Ã‚Â©dio
...
function estadoBarraToner(percentagem, cor = "black") {
  ...
  if (cor === "waste") {
    if (value >= 85) return "CrÃƒÆ’Ã‚Â­tico";
    if (value >= 65) return "Alto";
    if (value >= 45) return "MÃƒÆ’Ã‚Â©dio";
    return "OK";
  }

  if (value <= 10) return "CrÃƒÆ’Ã‚Â­tico";
  if (value <= 25) return "Baixo";
  if (value <= 50) return "MÃƒÆ’Ã‚Â©dio";
  return "Bom";
}
...
function gerarHTMLToners(info) {
  ...
  if (residueItem) {
    blocks.push(gerarHTMLBarraToner(residueItem.percent, residueItem.label || "ResÃƒÆ’Ã‚Â­duo", "waste"));
  }
...
function aplicarConfigNotificacoesApp(config = {}) {
  if (APP_NOTIFICATIONS_REBUILD_MODE) {
    ...
    setNotificationServiceText("notifyServiceStatus", "Em reconstruÃ§Ã£o", "warn");
    ...
    setNotificationServiceText("notifyCredentialsDetail", "Nova pÃ¡gina dedicada em preparaÃ§Ã£o");
    return;
  }
...
async function pedirPermissaoNotificacoesApp(options = {}) {
  ...
  // O registo nativo do Windows sÃ³ serve para teste local e nÃ£o recebe com a app fechada.

  if (!("Notification" in window)) {
    mostrarMensagem("Este dispositivo nÃƒÆ’Ã‚Â£o suporta notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes Web.", "erro");
    return;
  }
  ...
  try {
    ...
    if (permission === "granted") {
      ...
      if (shouldRegisterDevice) {
        ...
        await enviarNotificacaoApp("App Braga", "NotificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes ativas neste dispositivo.", "test-web", { force: true });
      }
    } else {
      mostrarMensagem("PermissÃƒÆ’Ã‚Â£o de notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes recusada.", "erro");
    }
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao ativar notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes.", "erro");
  }
}
...
async function guardarConfigNotificacoesApp(overrides = null) {
  ...
  if (!window.db || !window.db.collection) {
    mostrarMensagem("Firebase indisponÃƒÆ’Ã‚Â­vel para guardar notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes.", "erro");
    return;
  }
  ...
  try {
    ...
    mostrarMensagem("NotificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes atualizadas.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao guardar notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes.", "erro");
  }
}
...
async function enviarNotificacaoApp(title, body, tag = "app-braga", options = {}) {
  ...
  try {
    ...
    console.error("Erro notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o:", error);
    return false;
  }
}
  ...
  if (appNotificationState.stockMin && typeof buildAlertasInteligentes === "function") {
    buildAlertasInteligentes()
      ...
      .forEach((item) => alerts.push({
        ...
        title: "Stock abaixo do mÃƒÆ’Ã‚Â­nimo",
  ...
  if (appNotificationState.maintenance && Array.isArray(manutencoesGlobal)) {
    manutencoesGlobal
      ...
      .forEach((item) => alerts.push({
        ...
        title: "ManutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o pendente",
...
async function notificarAlteracaoRealtimeApp(collectionKey, snapshot) {
  ...
  const config = {
    ...
    manutencoes: {
      title: "ManutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o atualizada",
      label: "ManutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes",
      url: "html/manutencao-impressoras.html"
    },
    printers: {
...
async function garantirFirebaseMessagingApp() {
  ...
  if (!firebase.messaging) throw new Error("Firebase Messaging indisponÃƒÆ’Ã‚Â­vel.");
  const supported = firebase.messaging.isSupported ? await Promise.resolve(firebase.messaging.isSupported()) : true;
  if (!supported) throw new Error("Este browser/dispositivo nÃƒÆ’Ã‚Â£o suporta Firebase Messaging.");
...
async function atualizarEstadoNotificacoesApp(showMessage = false) {
  ...
  setNotificationServiceText("notifyServiceStatus", ok ? "Web pronta" : "PermissÃƒÆ’Ã‚Â£o pendente", ok ? "ok" : "warn");
  setNotificationServiceText("notifyServiceDetail", ok ? "A aguardar primeiro envio das Cloud Functions" : "Ativa as notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes neste dispositivo");
...
async function enviarAlertaGeralNotificacoesApp() {
  try {
    if (!window.db?.collection) throw new Error("Firebase indisponÃ­vel.");
    const settings = await carregarCloudNotificacoesApp();
    if (!settings || settings.enabled === false) throw new Error("Ativa primeiro as notificaÃ§Ãµes cloud.");
    ...
    const result = await enviarCloudPushNotificacoesApp({
      title: "ðŸš¨ Alerta geral - App Braga",
      body,
...
async function restaurarRegistoPushAtualApp() {
  ...
  try {
    ...
    if (permission !== "granted") {
      setNotificationTokenStatus(permission === "denied" ? "PermissÃƒÆ’Ã‚Â£o bloqueada" : "Sem permissÃƒÆ’Ã‚Â£o", permission === "denied" ? "bad" : "warn");
      return;
    }
    ...
    console.warn("NÃƒÆ’Ã‚Â£o foi possÃƒÆ’Ã‚Â­vel restaurar registo push:", error);
  } finally {
  ...
  if (output) {
    output.innerHTML = `
      <div class="printer-toner-box">
        <div class="printer-toner-head">
          <span class="printer-toner-title">ConsumÃƒÆ’Ã‚Â­veis</span>
  ...
  host.innerHTML = `
    <div class="printer-history-card">
      <div class="section-header">
        <div>
          <h3>${item.modelo} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${item.serie}</h3>
          <p class="section-subtitle">${item.armazem} Ãƒâ€šÃ‚Â· ${item.localizacao}</p>
        </div>
      </div>
      ...
      <div class="history-mini-grid">
        ...
        <div class="summary-card">
          <h4>ÃƒÆ’Ã…Â¡ltimo Registo</h4>
          <div class="meta-line">${ultimo ? `${ultimo.cor || "-"} Ãƒâ€šÃ‚Â· ${ultimo.data || "Sem Data"}` : "Sem registos"}</div>
        </div>
      </div>
      ...
      <div class="printer-history-items">
        ${itens.length ? itens.slice(0,8).map(h => `
          <div class="printer-history-item">
            ...
            <div class="meta-line">LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: <span class="meta-value">${h.localizacao || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o"}</span></div>
          </div>
        `).join("") : `<div class="panel empty-state"><h3>Sem histÃƒÆ’Ã‚Â³rico para esta impressora</h3><p>Quando houver movimentos associados, aparecem aqui.</p></div>`}
      </div>
    </div>
  `;
}
...
function renderImpressoras(lista = impressorasData) {
  ...
  const problema = impressorasData.filter(i => {
    ...
    return e === "Pendente" || e === "Em reparaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o";
  }).length;
      ...
      <tr>
        ...
        <td>
          ...
          <div class="table-actions" style="margin-top:8px;">
            ...
            <button class="action-btn manut" onclick='abrirManutencaoDireta(${JSON.stringify(item)})'>ManutenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</button>
            <button class="action-btn" onclick='abrirHistoricoImpressora(${JSON.stringify(item)})'>HistÃƒÆ’Ã‚Â³rico</button>
...
function badgeUser(valor) {
  return valor ? `<span class="badge ok">Sim</span>` : `<span class="badge livre">NÃƒÆ’Ã‚Â£o</span>`;
}
...
function setCookieAppBraga(name, value, maxAgeSeconds = null) {
  ...
  document.cookie = `${name}=${encodeURIComponent(String(value || ""))}${age}; Path=/; SameSite=Lax`;
    ...
    console.warn("Cookie indisponÃƒÆ’Ã‚Â­vel", error);
  }
}
...
function setSessionAppBraga(name, value) {
  try {
    ...
    console.warn("SessÃƒÆ’Ã‚Â£o temporÃƒÆ’Ã‚Â¡ria indisponÃƒÆ’Ã‚Â­vel", error);
  }
}
...
async function guardarResolucaoApp(value) {
  ...
  try {
    ...
    mostrarMensagem("ResoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o atualizada.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao guardar resoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
  }
}
...
function renderAppLockOverlayContent(overlay) {
  ...
  overlay.innerHTML = `
    <div class="app-lock-card">
      ...
      <h2>APP bloqueada</h2>
      <p>${canUseBio ? "Usa Face ID, impressÃƒÆ’Ã‚Â£o digital ou o PIN disponÃƒÆ’Ã‚Â­vel." : "Introduz o PIN para continuar."}</p>
      ${canUseBio ? `<button class="primary-btn biometric-unlock-btn" type="button" onclick="desbloquearAppComBiometria()">Face ID / impressÃƒÆ’Ã‚Â£o digital</button>` : ""}
...
async function ativarBiometriaApp() {
  if (!window.db || !window.db.collection) return mostrarMensagem("Firebase indisponÃƒÆ’Ã‚Â­vel.", "erro");
  if (!webAuthnDisponivelApp()) return mostrarMensagem("Este dispositivo nÃƒÆ’Ã‚Â£o suporta Face ID / fingerprint nesta app.", "erro");
  try {
    ...
    if (!credential?.rawId) return mostrarMensagem("NÃƒÆ’Ã‚Â£o foi possÃƒÆ’Ã‚Â­vel ativar biometria.", "erro");
    const biometricCredentialId = base64UrlFromBufferApp(credential.rawId);
    await window.db.collection("config").doc("layout").set({
      biometricEnabled: true,
      biometricCredentialId,
      biometricLabel: "Face ID / impressÃƒÆ’Ã‚Â£o digital",
    ...
    console.error(error);
    mostrarMensagem("AtivaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o biomÃƒÆ’Ã‚Â©trica cancelada ou indisponÃƒÆ’Ã‚Â­vel.", "erro");
  }
}

async function desbloquearAppComBiometria() {
  if (!webAuthnDisponivelApp()) return mostrarMensagem("Biometria indisponÃƒÆ’Ã‚Â­vel neste dispositivo.", "erro");
  if (!appSecurityState.biometricCredentialId) return mostrarMensagem("Ativa primeiro a biometria nas ConfiguraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes.", "erro");
  try {
...
async function guardarMetodoEntradaApp(value) {
  ...
  try {
    ...
    mostrarMensagem("MÃƒÆ’Ã‚Â©todo de entrada atualizado.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao guardar mÃƒÆ’Ã‚Â©todo de entrada.", "erro");
  }
}
...
async function verificarSistemasApp() {
  ...
  setHealthStatus("healthDevice", window.appBragaDeviceType || (document.body.classList.contains("device-phone") ? "TelemÃƒÆ’Ã‚Â³vel" : (document.body.classList.contains("device-tablet") ? "Tablet" : "PC")), "ok");
  ...
  setHealthStatus("healthFirebase", window.firebase ? "Carregado" : "IndisponÃƒÆ’Ã‚Â­vel", window.firebase ? "ok" : "bad");
  setHealthStatus("healthAuth", window.firebase?.auth ? "Carregado" : "IndisponÃƒÆ’Ã‚Â­vel", window.firebase?.auth ? "ok" : "warn");

  if (!window.db || typeof window.db.collection !== "function") {
    setHealthStatus("healthFirestore", "IndisponÃƒÆ’Ã‚Â­vel", "bad");
    return;
  }
  ...
  try {
    ...
    console.error("Erro no diagnÃƒÆ’Ã‚Â³stico Firestore:", error);
...
function initDeviceViewportMode() {
  const apply = () => {
    ...
    /*
      ...
      Agora:
      - Android + touch + ecrÃƒÆ’Ã‚Â£ grande = tablet
      - iPad = tablet
      - telemÃƒÆ’Ã‚Â³veis continuam phone
      - sÃƒÆ’Ã‚Â³ ÃƒÆ’Ã‚Â© PC quando NÃƒÆ’Ã†â€™O ÃƒÆ’Ã‚Â© Android/iPad touch tablet
    */
    const isPhone =
      isIosPhone ||
    ...
    window.appBragaDeviceType = isPhone
      ? (isAndroid ? "Android TelemÃƒÆ’Ã‚Â³vel" : "iPhone/TelemÃƒÆ’Ã‚Â³vel")
...
function renderRadios() {
  ...
  listaNode.innerHTML = lista.length ? lista.map((item) => {
    ...
    <article class="radio-card" data-radio-id="${safeRefHtml(item.id)}" onclick="atualizarRadioSelecionado('${safeRefHtml(item.id)}')">
      ...
      <div class="radio-card-main">
        ...
        <small>MAC ${safeRefHtml(item.mac || "-")} Ãƒâ€šÃ‚Â· SÃƒÆ’Ã‚Â©rie ${safeRefHtml(item.serial || item.numeroSerie || "-")}</small>
        <div class="radio-status-pill ${assigned ? "assigned" : "available"}">${assigned ? "AtribuÃƒÆ’Ã‚Â­do" : "DisponÃƒÆ’Ã‚Â­vel"}</div>
        <div class="radio-card-user">${assigned ? `User: ${safeRefHtml(currentUser)}` : "Sem user atribuÃƒÆ’Ã‚Â­do"}</div>
        ${assignedAt ? `<small>AtribuÃƒÆ’Ã‚Â­do em ${safeRefHtml(assignedAt)}</small>` : ""}
  ...
  `;
  }).join("") : `<div class="reference-empty">Sem rÃƒÆ’Ã‚Â¡dios registados na Firestore.</div>`;
  ...
  if (resumoNode) {
    resumoNode.innerHTML = records.length ? records.map((record) => {
      ...
      <div class="weekly-radio-row compact radio-record-row">
        <div>
          ...
          <small>${usedCount}/${assignments.length} rÃƒÆ’Ã‚Â¡dios com user associado</small>
        </div>
    ...
    `;
    }).join("") : `<div class="reference-empty">Ainda nÃƒÆ’Ã‚Â£o existem registos semanais.</div>`;
  }
...
function initRadiosPage() {
  ...
  if (!dbRef || typeof dbRef.collection !== "function") {
    const listaNode = document.getElementById("listaRadios");
    if (listaNode) listaNode.innerHTML = `<div class="reference-empty">Firebase indisponÃƒÆ’Ã‚Â­vel. Confirma a ligaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o da app.</div>`;
    return;
  }
  ...
  unsubscribeRadios = dbRef.collection("radios").onSnapshot((snapshot) => {
    ...
    const listaNode = document.getElementById("listaRadios");
    if (listaNode) listaNode.innerHTML = `<div class="reference-empty">Erro ao carregar rÃƒÆ’Ã‚Â¡dios da Firestore.</div>`;
  });
...
function abrirModalRadio(id = null) {
  ...
  if (title) title.textContent = id ? "Editar RÃƒÆ’Ã‚Â¡dio" : "Novo RÃƒÆ’Ã‚Â¡dio";
...
async function guardarRadio() {
  ...
  if (!nome) {
    mostrarMensagem("Preenche o nome do rÃƒÆ’Ã‚Â¡dio.", "erro");
    return;
  }
  ...
  try {
    if (radioEditId) {
      ...
      mostrarMensagem("RÃƒÆ’Ã‚Â¡dio atualizado.");
    } else {
      await window.db.collection("radios").add({ ...payload, createdAt: Date.now() });
      mostrarMensagem("RÃƒÆ’Ã‚Â¡dio criado.");
    }
    fecharModalRadio();
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao guardar rÃƒÆ’Ã‚Â¡dio.", "erro");
  }
}
...
async function apagarRadio(id) {
  try {
    ...
    mostrarMensagem("RÃƒÆ’Ã‚Â¡dio apagado.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao apagar rÃƒÆ’Ã‚Â¡dio.", "erro");
  }
}
...
function renderRadioWeeklyRecordDetails(recordId) {
  ...
  if (!record) {
    detalheNode.innerHTML = `<div class="reference-empty">Registo nÃƒÆ’Ã‚Â£o encontrado.</div>`;
    return;
  }
  ...
  detalheNode.innerHTML = `
    ...
    ${assignments.length ? assignments.map((item) => {
      ...
      <div class="weekly-radio-row">
        <strong>${safeRefHtml(item.radioNome || "RÃƒÆ’Ã‚Â¡dio")}</strong>
    ...
    `;
    }).join("") : `<div class="reference-empty">Este registo nÃƒÆ’Ã‚Â£o tem rÃƒÆ’Ã‚Â¡dios associados.</div>`}
  `;
}
...
function renderRadioWeeklyForm() {
  ...
  if (label) label.textContent = editRecord ? `${getRadioWeeklyRecordId(editRecord)} Ãƒâ€šÃ‚Â· ${weekInfo.label}` : weekInfo.label;
  ...
  rows.innerHTML = radiosData.length ? radiosData.map((radio) => {
    ...
    `;
  }).join("") : `<div class="reference-empty">Cria rÃƒÆ’Ã‚Â¡dios primeiro para conseguires fazer o registo semanal.</div>`;
}
...
function abrirEditarRegistoSemanalRadios(recordId) {
  const record = radioWeeklyRecords.find(item => item.id === recordId || item.recordId === recordId);
  if (!record) return mostrarMensagem("Registo semanal nÃƒÆ’Ã‚Â£o encontrado.", "erro");
...
async function guardarRegistoSemanalRadios() {
  if (!window.db) return mostrarMensagem("Firebase indisponÃƒÆ’Ã‚Â­vel.", "erro");
...
function renderInformacoes() {
  ...
  lista.innerHTML = listaFiltrada.length ? listaFiltrada.map(item => `
    <article class="info-list-item ${informacaoSelecionada === item.id ? "active" : ""}">
      <strong>${safeRefHtml(item.titulo || "Sem tÃƒÆ’Ã‚Â­tulo")}</strong>
      <span>${safeRefHtml(item.obs || "Sem observaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes")}</span>
  ...
  `).join("") : `<div class="info-empty">Ainda sem informaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes guardadas na Firebase.</div>`;
}
...
function initInformacoesPage() {
  ...
  if (!window.db || typeof window.db.collection !== "function") {
    const lista = document.getElementById("informacoesLista");
    if (lista) lista.innerHTML = `<div class="info-empty">Firebase indisponÃƒÆ’Ã‚Â­vel. Confirma a ligaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o da app.</div>`;
    return;
  }
  ...
  unsubscribeInformacoes = window.db.collection("informacoes").onSnapshot((snapshot) => {
    ...
    const lista = document.getElementById("informacoesLista");
    if (lista) lista.innerHTML = `<div class="info-empty">Erro ao carregar informaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes da Firebase.</div>`;
  });
}
...
async function adicionarInformacao() {
  ...
  if (!normalizarTexto(titulo) && !normalizarTexto(obs)) {
    mostrarMensagem("Preenche pelo menos um tÃƒÆ’Ã‚Â­tulo ou observaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
    return;
  }

  if (!window.db || typeof window.db.collection !== "function") {
    mostrarMensagem("Firebase indisponÃƒÆ’Ã‚Â­vel.", "erro");
    return;
  }
  ...
  try {
    if (informacaoSelecionada) {
      ...
      mostrarMensagem("InformaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o atualizada.");
    } else {
      ...
      mostrarMensagem("InformaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o criada.");
    }
    ...
    console.error(error);
    mostrarMensagem("Erro ao guardar informaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
  }
}
...
function verInformacao(id) {
  ...
  if (!item) return mostrarMensagem("InformaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o nÃƒÆ’Ã‚Â£o encontrada.", "erro");
  const modal = document.getElementById("infoModal");
  const titulo = document.getElementById("modalInfoTitulo");
  const descricao = document.getElementById("modalInfoDescricao");
  if (titulo) titulo.textContent = item.titulo || "InformaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o";
  if (descricao) descricao.textContent = item.obs || "Sem observaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes";
...
async function apagarInformacao(id) {
  const item = informacoesData.find(info => info.id === id);
  if (!item) return mostrarMensagem("InformaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o nÃƒÆ’Ã‚Â£o encontrada.", "erro");
  if (!window.confirm(`Apagar "${item.titulo || "esta informaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o"}"?`)) return;
  try {
    ...
    mostrarMensagem("InformaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o apagada.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao apagar informaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.", "erro");
  }
}

function verInformacaoSelecionada() {
  if (!informacaoSelecionada) return mostrarMensagem("Seleciona uma informaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o primeiro.", "erro");
...
function abrirAtribuirRadio(id) {
  ...
  if (!radio) return mostrarMensagem("RÃƒÆ’Ã‚Â¡dio nÃƒÆ’Ã‚Â£o encontrado.", "erro");
  radioAssignId = id;

  const title = document.getElementById("radioAssignTitle");
  if (title) title.textContent = `Atribuir ${radio.nome || "RÃƒÆ’Ã‚Â¡dio"}`;
...
async function guardarAtribuirRadio() {
  ...
  try {
    ...
    fecharAtribuirRadio();
    mostrarMensagem("RÃƒÆ’Ã‚Â¡dio atribuÃƒÆ’Ã‚Â­do.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao atribuir rÃƒÆ’Ã‚Â¡dio.", "erro");
  }
}
...
async function devolverRadio(id) {
  ...
  if (!userNome && !window.confirm("Este rÃƒÆ’Ã‚Â¡dio nÃƒÆ’Ã‚Â£o tem user associado. Marcar como devolvido mesmo assim?")) return;
  ...
  try {
    ...
    await guardarHistoricoRadio(radio, "devolvido", { userNome, obs: "DevoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o manual" });
    mostrarMensagem("RÃƒÆ’Ã‚Â¡dio devolvido.");
  } catch (error) {
    console.error(error);
    mostrarMensagem("Erro ao devolver rÃƒÆ’Ã‚Â¡dio.", "erro");
  }
}
...
function abrirHistoricoRadio(id) {
  ...
  if (title) title.textContent = `HistÃƒÆ’Ã‚Â³rico Ãƒâ€šÃ‚Â· ${radio.nome || "RÃƒÆ’Ã‚Â¡dio"}`;
  if (list) list.innerHTML = `<div class="reference-empty">A carregar histÃƒÆ’Ã‚Â³rico...</div>`;
  ...
  if (!window.db?.collection) {
    if (list) list.innerHTML = `<div class="reference-empty">Firebase indisponÃƒÆ’Ã‚Â­vel.</div>`;
    return;
  }
    ...
    .onSnapshot((snapshot) => {
      ...
      list.innerHTML = items.length ? items.map((item) => `
        <div class="radio-history-item">
          <strong>${safeRefHtml(item.tipo === "atribuido" ? "AtribuÃƒÆ’Ã‚Â­do" : item.tipo === "devolvido" ? "Devolvido" : item.tipo || "Evento")}</strong>
      ...
      `).join("") : `<div class="reference-empty">Sem histÃƒÆ’Ã‚Â³rico para este rÃƒÆ’Ã‚Â¡dio.</div>`;
    }, (error) => {
      console.error(error);
      if (list) list.innerHTML = `<div class="reference-empty">Erro ao carregar histÃƒÆ’Ã‚Â³rico.</div>`;
    });
}
...
function atualizarRadioSelectOptions() {
  ...
  select.innerHTML = `<option value="">Selecionar rÃƒÆ’Ã‚Â¡dio...</option>` + lista.map((radio) => {
    const label = `${radio.nome || "RÃƒÆ’Ã‚Â¡dio"}${radio.mac ? " Ãƒâ€šÃ‚Â· " + radio.mac : ""}${radio.serial ? " Ãƒâ€šÃ‚Â· " + radio.serial : ""}`;
...
function atualizarRadioSelecionado(id = null) {
  ...
  if (info) {
    if (!radio) {
      info.textContent = "Seleciona um rÃƒÆ’Ã‚Â¡dio para mexer.";
    } else {
      const user = radioCurrentUserName(radio);
      info.textContent = `${radio.nome || "RÃƒÆ’Ã‚Â¡dio"} Ãƒâ€šÃ‚Â· MAC ${radio.mac || "-"} Ãƒâ€šÃ‚Â· SÃƒÆ’Ã‚Â©rie ${radio.serial || radio.numeroSerie || "-"} Ãƒâ€šÃ‚Â· ${user ? "User: " + user : "DisponÃƒÆ’Ã‚Â­vel"}`;
    }
  }
}
...
function radioAcaoSelecionada(acao) {
  const radio = getRadioSelected();
  if (!radio) {
    mostrarMensagem("Seleciona primeiro um rÃƒÆ’Ã‚Â¡dio.", "erro");
    return;
  }
...
function updateEnterpriseDashboard() {
  ...
  const ticketsAbertos = Array.isArray(manutencoesGlobal)
    ? manutencoesGlobal.filter(item => item.estado === "Pendente" || item.estado === "Em reparaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o").length
    : 0;
...
function bindPrintersFirebaseRealtime() {
  ...
  appBragaBindFirestoreListener("printers-realtime", true, () => db.collection("printers").onSnapshot((snap) => {
    ...
    console.error("Erro ao ler coleÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o printers:", error);
  }));
}
...
abrirIP = function(ip) {
  // No tablet/web o IP fica sÃƒÆ’Ã‚Â³ de leitura
  const webMode = !(window.electronAPI && window.electronAPI.getTonerSNMP);
  if (webMode) return;
...
/* =========================
   DIAGNÃƒÆ’Ã¢â‚¬Å“STICO DO TONER
========================= */
const tonerDiagnosticsState = {
  running: false,
  lastRunAt: null,
  source: "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â",
  successCount: 0,
  totalCount: 0,
  status: "idle",
  log: []
...
function renderTonerDiagnostics() {
  ...
  sourceEl.textContent = tonerDiagnosticsState.source || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â";
  ...
  if (tonerDiagnosticsState.running) {
    ...
    summaryEl.textContent = "ÃƒÆ’Ã¢â€šÂ¬ espera de teste";
  } else {
  ...
  logEl.innerHTML = tonerDiagnosticsState.log.map(item => `
    <div class="diagnostics-log-item">
      ...
      <strong>${item.ip}</strong> Ãƒâ€šÃ‚Â· ${item.message}
    </div>
  `).join("");
}
...
function summarizeTonerInfo(info) {
  ...
  if (Array.isArray(info.colors) && info.colors.length) {
    return info.colors.map(c => `${c.label || c.key}: ${typeof c.percent === "number" ? Math.round(c.percent) : "N/D"}%`).join(" Ãƒâ€šÃ‚Â· ");
  }
...
function montarTextoLocalizacaoStable(item) {
  if (!item) return "Sem LocalizaÃƒÂ§ÃƒÂ£o";
  const serie = String(item.serie || "").trim();
  const armazem = String(item.armazem || "Braga").trim();
  const local = String(item.localizacao || item.local || "Sem LocalizaÃƒÂ§ÃƒÂ£o").trim();
...
async function processarTextoLidoStable(textoLido) {
  ...
  mostrarMensagem("CÃƒÆ’Ã‚Â³digo nÃƒÆ’Ã‚Â£o reconhecido para preenchimento automÃƒÆ’Ã‚Â¡tico.", "erro");
  return false;
}
...
async function startScannerStable() {
  ...
  if (!reader) {
    mostrarMensagem("Zona do scanner nÃƒÆ’Ã‚Â£o encontrada.", "erro");
    return;
  }
  if (scannerAtivoStable) {
    mostrarMensagem("A cÃƒÆ’Ã‚Â¢mara jÃƒÆ’Ã‚Â¡ estÃƒÆ’Ã‚Â¡ aberta.", "erro");
    return;
  }
  ...
  try {
    ...
    await scannerInstanceStable.start(
      ...
      async (decodedText) => {
        enhanceScannerStatus("CÃƒÆ’Ã‚Â³digo lido. A processar automaticamente...");

        // 1Ãƒâ€šÃ‚Âº tenta usar o QR como etiqueta de toner existente em Stock.
        // Se encontrar, passa automaticamente de Stock para HistÃƒÆ’Ã‚Â³rico.
        const passouStockHistorico = await usarPorCodigoEtiquetaToner(decodedText);
        if (!passouStockHistorico) {
          // Se nÃƒÆ’Ã‚Â£o for QR de stock, mantÃƒÆ’Ã‚Â©m o comportamento antigo do scanner.
    ...
    enhanceScannerStatus("CÃƒÆ’Ã‚Â¢mara iniciada. ÃƒÆ’Ã¢â€šÂ¬ espera de leitura inteligente.");
    mostrarMensagem("CÃƒÆ’Ã‚Â¢mara iniciada.");
  } catch (e) {
    console.error("Erro ao iniciar scanner:", e);
    mostrarMensagem("NÃƒÆ’Ã‚Â£o foi possÃƒÆ’Ã‚Â­vel abrir a cÃƒÆ’Ã‚Â¢mara.", "erro");
  }
}
...
function abrirOCRStable() {
  ...
  if (!input) {
    mostrarMensagem("Input OCR nÃƒÆ’Ã‚Â£o encontrado.", "erro");
    return;
  }
  input.value = "";
  input.click();
...
async function processarOCRInputStable(event) {
  ...
  try {
    ...
    const resumo = [
      ...
      dados.serie ? `SÃƒÆ’Ã‚Â©rie: ${dados.serie}` : ""
    ].filter(Boolean).join(" | ");

    mostrarOCRStatusStable(resumo || "A folha foi lida, mas nÃƒÆ’Ã‚Â£o encontrei dados suficientes.");
    mostrarMensagem(ok ? "Folha lida com sucesso." : "NÃƒÆ’Ã‚Â£o encontrei dados suficientes na folha.", ok ? "sucesso" : "erro");
  } catch (e) {
...
function garantirTesseractStable() {
  ...
  window.__appBragaTesseractPromise = new Promise((resolve, reject) => {
    ...
    script.async = true;
    script.onload = () => resolve();
    script.onerror = () => reject(new Error("Biblioteca OCR nÃ£o carregada."));
    document.head.appendChild(script);
  });
...
function confirmarSerie3DigitosStable() {
  ...
  if (valor.length !== 3) {
    mostrarMensagem("Introduza exatamente 3 dÃƒÆ’Ã‚Â­gitos.", "erro");
    return;
  }

  const printer = procurarImpressoraPorUltimos3DigitosStable(valor);
  if (!printer) {
    mostrarMensagem("Nenhuma impressora encontrada com esses 3 dÃƒÆ’Ã‚Â­gitos.", "erro");
    return;
  }
  ...
  mostrarMensagem("LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o selecionada com sucesso.");
}
...
/* =========================
   ETIQUETA WORD AUTOMÃƒÆ’Ã‚ÂTICA
========================= */
...
function extrairDadosEtiquetaWord() {
  ...
  if (parts.length >= 3) {
    ...
    localCurto = loc || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o";
  }

  const dataEtiqueta = formatDatePTAppBraga(dataFolha || dataScan);

  return {
    serie: serie || "SEM SÃƒÆ’Ã¢â‚¬Â°RIE",
    localCurto: localCurto || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o",
    armazem: armazem || "",
...
async function gerarWordEtiquetaFromForm(auto = false) {
  try {
    if (typeof docx === "undefined") {
      if (!auto) mostrarMensagem("Biblioteca Word nÃƒÆ’Ã‚Â£o carregada.", "erro");
      return false;
    }
...
/* =========================
   PORTAS FIREBASE FALLBACK + MIGRAÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O
========================= */
async function migrarPortasParaFirebase() {
  if (!window.db) {
    mostrarMensagem("Firebase nÃƒÆ’Ã‚Â£o estÃƒÆ’Ã‚Â¡ disponÃƒÆ’Ã‚Â­vel.", "erro");
    return;
  }
  ...
  try {
    ...
    if (!snap.empty) {
      mostrarMensagem("A coleÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o portas jÃƒÆ’Ã‚Â¡ tem dados. MigraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o nÃƒÆ’Ã‚Â£o necessÃƒÆ’Ã‚Â¡ria.");
      return;
    }
    ...
    mostrarMensagem("MigraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o das portas concluÃƒÆ’Ã‚Â­da com sucesso.");
  } catch (e) {
    console.error(e);
...
function editarPorta(ref) {
  ...
  if (!item) return mostrarMensagem("Porta nÃƒÆ’Ã‚Â£o encontrada.", "erro");
  portaEditRef = ref;
...
function editarUser(ref) {
  ...
  if (!item) return mostrarMensagem("User nÃƒÆ’Ã‚Â£o encontrado.", "erro");
  userEditRef = ref;
  const fields = ["nome","zona","user_pc_eye","pass_remote","pass_eye_peak","op_pistola","pass_pistola","nome_pc","teamviewer","user_mo365","pw_mo365","email_bragalis","pass_bragalis"];
...
<div class="content">
  ...
  ${linhas || `
    ...
    <div class="print-row">
      ...
      <div class="print-value">
        Este user nÃƒÆ’Ã‚Â£o tem campos preenchidos.
      </div>
...
`;
  ...
  if (!frameWindow) {
    ...
    return mostrarMensagem(
      "Erro ao abrir impressÃƒÆ’Ã‚Â£o",
      "erro"
    );
...
function editarPistola(ref) {
  ...
  if (!item) return mostrarMensagem("Pistola nÃƒÆ’Ã‚Â£o encontrada.", "erro");
  pistolaEditRef = ref;
...
async function guardarNovaImpressora() {
  ...
  if (!normalizarTexto(payload.modelo) || !normalizarTexto(payload.serie) || !normalizarTexto(payload.ip)) {
    return mostrarMensagem("Preenche pelo menos Modelo, SÃƒÆ’Ã‚Â©rie e IP.", "erro");
  }
...
function getTopConsumoEquipamentos(limit = 4) {
  const map = new Map();
  historicoGlobal.forEach(item => {
    const key = `${item.equipamento || "-"} Ãƒâ€šÃ‚Â· ${item.localizacao || "-"}`;
...
function getTopProblemasDoDia(limit = 3) {
  ...
  if (buckets.critical > 0) {
    problems.push(`Existem ${buckets.critical} impressoras em estado crÃƒÆ’Ã‚Â­tico.`);
  }
  if (buckets.warning > 0) {
    problems.push(`Existem ${buckets.warning} impressoras em zona de atenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.`);
  }
  if (topLocs.length) {
    problems.push(`Maior pressÃƒÆ’Ã‚Â£o recente em ${topLocs[0][0]} com ${topLocs[0][1]} movimentos.`);
  }
  if (ultimos.length) {
    const u = ultimos[0];
    problems.push(`ÃƒÆ’Ã…Â¡ltimo movimento: ${u.equipamento || "-"} Ãƒâ€šÃ‚Â· ${u.cor || "-"} Ãƒâ€šÃ‚Â· ${u.localizacao || "-"}.`);
  }
...
function getPrioridadeMaximaGestor(limit = 4) {
  ...
  impressorasData.forEach(item => {
    ...
    if (crit.length) {
      rows.push({
        label: `${item.modelo} Ãƒâ€šÃ‚Â· ${item.localizacao}`,
...
function renderModoGestorExtremo() {
    ...
    board.innerHTML = `
      <div class="gestor-grid-hero">
        <div class="gestor-hero-card">
          ...
          <div class="gestor-hero-value">${buckets.critical > 0 ? "PressÃƒÆ’Ã‚Â£o" : "EstÃƒÆ’Ã‚Â¡vel"}</div>
          <div class="gestor-hero-note">VisÃƒÆ’Ã‚Â£o imediata da operaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o para decidir onde agir primeiro.</div>
          <div class="gestor-chip-row">
            <span class="gestor-chip red">CrÃƒÆ’Ã‚Â­ticos: ${buckets.critical}</span>
            <span class="gestor-chip yellow">AtenÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: ${buckets.warning}</span>
        ...
        <div class="gestor-card">
          ...
          <div class="meta-line">Total de registos usados no histÃƒÆ’Ã‚Â³rico.</div>
        </div>
        <div class="gestor-card">
          <h4>Capacidade atual</h4>
          <div class="gestor-mini-value">${stockGlobal.length}</div>
          <div class="meta-line">Itens disponÃƒÆ’Ã‚Â­veis agora em stock.</div>
        </div>
        <div class="gestor-card">
  ...
  if (prioridade) {
    prioridade.innerHTML = maxRows.length
      ...
      : `<div class="gestor-priority-card"><h4>Sem prioridade mÃƒÆ’Ã‚Â¡xima</h4><div class="meta-line">NÃƒÆ’Ã‚Â£o existem impressoras com toner a 0% neste momento.</div></div>`;
  }
    ...
    consumo.innerHTML = `
      <div class="gestor-card">
        ...
        <ul class="gestor-list">
          ${topLocs.length ? topLocs.map(([k,v]) => `<li>${k} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${v} movimentos</li>`).join("") : "<li>Sem dados suficientes</li>"}
        </ul>
      </div>
      <div class="gestor-card">
        <h4>Top Equipamentos</h4>
        <ul class="gestor-list">
          ${topEquip.length ? topEquip.map(([k,v]) => `<li>${k} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${v}</li>`).join("") : "<li>Sem dados suficientes</li>"}
        </ul>
      </div>
    `;
  }
  ...
  if (problemas) {
    problemas.innerHTML = topProb.length
      ? topProb.map(txt => `<div class="gestor-alert-card"><h4>Ponto de gestÃƒÆ’Ã‚Â£o</h4><div class="meta-line">${txt}</div></div>`).join("")
      : `<div class="gestor-alert-card"><h4>Sem alertas do dia</h4><div class="meta-line">Ainda nÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ dados suficientes para destacar problemas.</div></div>`;
  }
}
...
const STOCK_MIN_DEFAULTS = {
  ...
  "Braga - Ilha 04": 1,
  "Braga - Ilha 05": 1,
  "Braga - BalcÃƒÆ’Ã‚Â£o 01": 2,
  "Braga - BalcÃƒÆ’Ã‚Â£o 02": 2,
  "Braga - Dep. Logistica": 2,
  "Braga - G/Encomendas": 1,
  "Braga - DevoluÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes": 1,
  "Braga - Escritorio": 1,
  "Vila Real - Ilha 01": 1,
  "Vila Real - Ilha 02": 1,
  "Vila Real - Ilha 03": 1,
  "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o": 1
};
...
function normalizeLocMin(loc) {
  const raw = String(loc || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o");
  if (raw.includes(" - ")) {
...
function renderStockMinimoPainel() {
  ...
  host.innerHTML = Object.keys(config).map(loc => {
    ...
    const estado = atual < minimo ? "Abaixo do mÃƒÆ’Ã‚Â­nimo" : atual === minimo ? "No mÃƒÆ’Ã‚Â­nimo" : "Acima do mÃƒÆ’Ã‚Â­nimo";
    return `<div class="stock-min-card"><strong>${loc}</strong><div class="meta-line">Atual: <span class="meta-value ${cls}">${atual}</span></div><div class="meta-line">MÃƒÆ’Ã‚Â­nimo: <span class="meta-value">${minimo}</span></div><div class="meta-line">${estado}</div></div>`;
  }).join("");
}
...
function guardarStockMinimoConfig() {
  ...
  mostrarMensagem("Stock mÃƒÆ’Ã‚Â­nimo guardado com sucesso.");
  tryRenderAppBraga(renderStockMinimoPainel);
  tryRenderAppBraga(renderAlertasInteligentes);
}
...
function resetStockMinimoConfig() {
  ...
  renderStockMinimoPainel();
  renderAlertasInteligentes();
  mostrarMensagem("Stock mÃƒÆ’Ã‚Â­nimo reposto.");
}
...
function exportarExcelStock() {
  if (!stockGlobal.length) return mostrarMensagem("NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ stock para exportar.", "erro");
...
function exportarExcelHistorico() {
  if (!historicoGlobal.length) return mostrarMensagem("NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ histÃƒÆ’Ã‚Â³rico para exportar.", "erro");
...
function exportarExcelTudo() {
  ...
  if (!rows.length) return mostrarMensagem("NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ dados para exportar.", "erro");
...
async function importBackupCompletoApp(event) {
  ...
  try {
    ...
    if (!selected.length) return mostrarMensagem("Backup sem dados importÃƒÆ’Ã‚Â¡veis.", "erro");
    if (!confirm(`Importar backup completo? Isto vai adicionar ${selected.reduce((sum, [, rows]) => sum + rows.length, 0)} registos ÃƒÆ’Ã‚Â s coleÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes.`)) return;
...
function abrirEditarStockModal(id) {
  ...
  if (!item) return mostrarMensagem("Item de stock nÃƒÆ’Ã‚Â£o encontrado.", "erro");
  stockEditModalId = id;
...
function abrirEditarHistoricoModal(id) {
  ...
  if (!item) return mostrarMensagem("HistÃƒÆ’Ã‚Â³rico nÃƒÆ’Ã‚Â£o encontrado.", "erro");
...
async function guardarEdicaoHistoricoModal() {
  ...
  try {
    ...
    mostrarMensagem("HistÃƒÆ’Ã‚Â³rico atualizado.");
  } catch (e) {
    console.error(e);
    mostrarMensagem("Erro ao atualizar histÃƒÆ’Ã‚Â³rico.", "erro");
  }
}
...
function buildAlertasInteligentes() {
  ...
  Object.keys(cfg).forEach(loc => {
    ...
    if (atual < minimo) rows.push({ tipo: "stock", titulo: loc, detalhe: `Stock abaixo do mÃƒÆ’Ã‚Â­nimo: ${atual}/${minimo}` });
  });
  ...
  if (typeof impressorasData !== "undefined" && typeof tonerInfoState !== "undefined") {
    impressorasData.forEach(item => {
      ...
      const colors = Array.isArray(info?.colors) ? info.colors : [];
      const crit = colors.filter(c => isTonerEmpty(c.percent));
      if (crit.length) rows.push({ tipo: "printer", titulo: `${item.modelo} Ãƒâ€šÃ‚Â· ${item.localizacao}`, detalhe: crit.map(c => `${c.label}: ${c.percent}%`).join(" | ") });
    });
  }
...
function renderAlertasInteligentes() {
  ...
  ["alertasInteligentesDashboard","alertasInteligentesImpressoras"].forEach(id => {
    const host = el(id);
    if (!host) return;
    host.innerHTML = rows.length ? rows.map(r => `<div class="alert-inteligente-card"><strong>${r.titulo}</strong><div class="meta-line">${r.detalhe}</div></div>`).join("") : `<div class="alert-inteligente-card"><strong>Sem alertas</strong><div class="meta-line">NÃƒÆ’Ã‚Â£o existem alertas inteligentes ativos.</div></div>`;
  });
}
...
function parseLocalizacaoEtiquetaShared(loc) {
  ...
  let serie = "";
  let localCurto = raw || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o";
  let armazem = "";
...
function montarPayloadEtiquetaPartilhada(extra = {}) {
  ...
  return {
    serie: info.serie || extra.serie || "SEM SÃƒÆ’Ã¢â‚¬Â°RIE",
    localCurto: info.localCurto || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o",
    armazem: info.armazem || extra.armazem || "",
    localizacao: info.localizacaoRaw || loc || "Sem LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o",
...
async function gerarWordEtiquetaPartilhada(dados, opts = {}) {
  try {
    if (typeof docx === "undefined") {
      mostrarMensagem("Biblioteca Word nÃƒÆ’Ã‚Â£o carregada.", "erro");
      return false;
    }
  ...
  host.innerHTML = items.map(t => `
    <div class="stock-card etiqueta-word-card">
      <label class="etiqueta-select-row">
        ...
        <span>Selecionar para impressÃƒÆ’Ã‚Â£o</span>
      </label>
      <div class="stock-id">${t.localCurto || t.localizacao || 'Etiqueta'}</div>
      <div class="meta-line">SÃƒÆ’Ã‚Â©rie: <span class="meta-value">${t.serie || '-'}</span></div>
      <div class="meta-line">ArmazÃƒÆ’Ã‚Â©m: <span class="meta-value">${t.armazem || '-'}</span></div>
      <div class="meta-line">LocalizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o: <span class="meta-value">${t.localizacao || '-'}</span></div>
      <div class="meta-line">Equipamento: <span class="meta-value">${t.equipamento || '-'}</span></div>
      <div class="meta-line">Cor: <span class="meta-value">${t.cor || '-'}</span></div>
      <div class="meta-line">Lote: <span class="meta-value">${t.lote || '-'}</span></div>
      <div class="meta-line">SDS Ref: <span class="meta-value">${t.sdsRef || '-'}</span></div>
      <div class="meta-line">CÃƒÆ’Ã‚Â³digo: <span class="meta-value">${t.codigoEtiqueta || '-'}</span></div>
...
function montarHtmlEtiquetaImpressao(item) {
  const linhas = [
    ...
    ["SÃƒÆ’Ã‚Â©rie", item.serie],
    ["ArmazÃƒÆ’Ã‚Â©m", item.armazem],
...
async function regerarEtiquetaWordPartilhada(id) {
  ...
  if (!item) return mostrarMensagem("Etiqueta nÃƒÆ’Ã‚Â£o encontrada.", "erro");
  try {
    ...
    setTimeout(() => {
      try {
        ...
        console.error(e);
        mostrarMensagem('Erro ao abrir a impressÃƒÆ’Ã‚Â£o.', 'erro');
      } finally {
        setTimeout(() => {
    ...
    console.error(e);
    mostrarMensagem('Erro ao preparar impressÃƒÆ’Ã‚Â£o.', 'erro');
  }
}
...
async function imprimirEtiquetasWordSelecionadas() {
  ...
  if (!items.length) return mostrarMensagem("As etiquetas selecionadas jÃƒÆ’Ã‚Â¡ nÃƒÆ’Ã‚Â£o existem.", "erro");
...
/* =========================================================
   APP BRAGA ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SIDEBAR BRINKA + DASHBOARD CLEAN
   ========================================================= */
(function(){
...
async function importarExcelFirebase(){
  ...
  if(!input){
    ...
    alert(
      "Input Excel nÃƒÆ’Ã‚Â£o encontrado."
    );
  ...
  try{
    ...
    input.onchange = async (event)=>{
      ...
      try{
        ...
        alert(
          "ImportaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o concluÃƒÆ’Ã‚Â­da: "
          + total +
          " users."
        );
...
async function importarUsersJSONFirebase(){
  ...
  try{
    ...
    if(!window.db){
      ...
      alert("Firebase nÃƒÆ’Ã‚Â£o iniciada.");
...
/* ORDENAÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O ALFANUMÃƒÆ’Ã¢â‚¬Â°RICA USERS */
setInterval(() => {
  try{
...
/* ===== ORGANIZAÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O ALFANUMÃƒÆ’Ã¢â‚¬Â°RICA ===== */
...
/* ===== ORDENAÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O ALFANUMÃƒÆ’Ã¢â‚¬Â°RICA SEGURA ===== */
...
window.editarPistola = function(id){
  ...
  if(!pistola){
    console.error("Pistola nÃƒÆ’Ã‚Â£o encontrada", id);
    return;
  }
...
window.guardarEdicaoPistola = async function(){
  ...
  try{
    ...
    if(!id){
      ...
      alert("ID da pistola invÃƒÆ’Ã‚Â¡lido");
      return;
...
window.verMaisPistola = function(id){
  ...
  if(!pistola){
    alert("Pistola nÃƒÆ’Ã‚Â£o encontrada");
    return;
  }
...
ArmazÃƒÆ’Ã‚Â©m: ${pistola.armazem || "-"}
...
window.editarPistola = function(id) {
  ...
  if (!pistola) {
    mostrarMensagem("Pistola nÃƒÆ’Ã‚Â£o encontrada.", "erro");
    return;
  }
  window.pistolaAtual = pistola;
...
window.guardarEdicaoPistola = async function() {
  ...
  if (!dados.nome && !dados.num) {
    mostrarMensagem("Preenche pelo menos o nÃƒÆ’Ã‚Âºmero ou o nome da pistola.", "erro");
    return;
  }
  ...
  container.innerHTML = items.length ? items.map((pistola, index) => {
    ...
    return `
      <div class="pc-card pistol-card">
        ...
        <div class="meta-line">NÃƒâ€šÃ‚Âº: <span class="meta-value">${safeRefHtml(pistola.num || "-")}</span></div>
        <div class="meta-line">Operador: <span class="meta-value">${safeRefHtml(pistola.operador || "-")}</span></div>
        <div class="meta-line">ArmazÃƒÆ’Ã‚Â©m: <span class="meta-value">${safeRefHtml(pistola.armazem || "-")}</span></div>
...
async function guardarModoTextoBotoes(mode) {
  ...
  try {
    ...
    mostrarMensagem("Cor do texto dos botÃƒÆ’Ã‚Âµes atualizada.");
  } catch (error) {
    console.error(error);
...
function tocarBipStockQr() {
  try {
    ...
    // Sem som se o browser bloquear ÃƒÆ’Ã‚Â¡udio.
  }
}
...
async function escolherCameraTraseiraStockQr() {
  try {
    ...
    if (!Array.isArray(cameras) || !cameras.length) {
      throw new Error("Nenhuma cÃƒÆ’Ã‚Â¢mera encontrada pelo navegador.");
    }
    ...
    console.error("Erro ao escolher cÃƒÆ’Ã‚Â¢mera:", error);
    throw error;
  }
}
...
function mensagemErroCameraStockQr(error) {
  ...
  if (/NotAllowed|Permission|denied/i.test(name + " " + message)) {
    return "CÃƒÆ’Ã‚Â¢mera bloqueada. Vai ÃƒÆ’Ã‚Â s definiÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes do Safari e permite a cÃƒÆ’Ã‚Â¢mera para este site.";
  }

  if (/NotFound|DevicesNotFound/i.test(name + " " + message)) {
    return "NÃƒÆ’Ã‚Â£o encontrei cÃƒÆ’Ã‚Â¢mera disponÃƒÆ’Ã‚Â­vel no iPhone.";
  }

  if (/NotReadable|TrackStart/i.test(name + " " + message)) {
    return "A cÃƒÆ’Ã‚Â¢mera estÃƒÆ’Ã‚Â¡ ocupada por outra app. Fecha a cÃƒÆ’Ã‚Â¢mera/WhatsApp e tenta outra vez.";
  }

  if (/Overconstrained|Constraint/i.test(name + " " + message)) {
    return "O iPhone recusou a cÃƒÆ’Ã‚Â¢mera traseira. Vou tentar outra cÃƒÆ’Ã‚Â¢mera.";
  }

  return "Erro ao abrir cÃƒÆ’Ã‚Â¢mera: " + (message || name || "erro desconhecido");
}
...
async function startStockQrScanner() {
  ...
  if (!reader) {
    mostrarMensagem("ÃƒÆ’Ã‚Ârea do scanner QR nÃƒÆ’Ã‚Â£o encontrada.", "erro");
    return;
  }
  ...
  try {
    ...
    console.error(error);
    mostrarMensagem("Biblioteca do scanner QR nÃƒÆ’Ã‚Â£o carregada.", "erro");
    setStockQrStatus("Biblioteca QR nÃƒÆ’Ã‚Â£o carregada. Verifica internet/HTTPS.", "erro");
    return;
  }

  if (stockQrScannerActive) {
    mostrarMensagem("Scanner QR jÃƒÆ’Ã‚Â¡ estÃƒÆ’Ã‚Â¡ ligado.", "erro");
    return;
  }
  ...
  try {
    ...
    await stockQrScannerInstance.start(
      ...
      async (decodedText) => {
        ...
        if (handled) {
          tocarBipStockQr();
          setStockQrStatus("Toner marcado como usado e movido para HistÃƒÆ’Ã‚Â³rico.", "ok");
          await stopStockQrScanner();
        } else {
          setStockQrStatus("QR lido, mas nÃƒÆ’Ã‚Â£o ÃƒÆ’Ã‚Â© uma etiqueta de toner vÃƒÆ’Ã‚Â¡lida.", "erro");
          mostrarMensagem("QR nÃƒÆ’Ã‚Â£o reconhecido como etiqueta de toner.", "erro");
        }
      },
      () => {}
    );
    ...
    setStockQrStatus("CÃƒÆ’Ã‚Â¢mera ligada. Aponta para o QR da etiqueta.");
    mostrarMensagem("CÃƒÆ’Ã‚Â¢mera QR ligada.");
  } catch (error) {
...
window.testarCamerasStockQr = async function(){
  try{
    ...
    console.log("CÃƒÆ’Ã‚Â¢meras disponÃƒÆ’Ã‚Â­veis:", cams);
    setStockQrStatus("CÃƒÆ’Ã‚Â¢meras encontradas: " + (cams || []).map(c => c.label || c.id).join(" | "));
    return cams;
  }catch(e){
    console.error(e);
...
/* =========================
   APP BRAGA v1.30.7 Ã¢â‚¬â€ DIAGNÃƒâ€œSTICO + UX SEGURO
   NÃƒÂ£o altera autenticaÃƒÂ§ÃƒÂ£o, roles ou estrutura Firebase.
========================= */
(function(){
  ...
  async function atualizarDashboardUtilAppBraga(){
    ...
    try{
      ...
      setTxt("dashHealthMini", critical ? "AtenÃƒÂ§ÃƒÂ£o" : "OK");
      setTxt("dashHealthMiniText", critical ? `${critical} erro(s) nas ÃƒÂºltimas leituras` : "Sem erros crÃƒÂ­ticos");
      ...
      if(mHost){
        ...
        const rows = [...pend.map(x=>({a:x.impressora||x.equipamento||"ManutenÃƒÂ§ÃƒÂ£o", b:x.estado||x.status||"Pendente"})), ...impProblem.map(x=>({a:x.nome||x.modelo||x.localizacao||"Impressora", b:x.estado||x.status||"AtenÃƒÂ§ÃƒÂ£o"}))].slice(0,5);
        mHost.innerHTML = rows.length ? rows.map(x => `<div class="mini-feed-item"><strong>${esc(x.a)}</strong><small>${esc(x.b)}</small></div>`).join("") : `<div class="mini-feed-item"><span>Sem pendÃƒÂªncias detetadas</span></div>`;
      }
  ...
  setTimeout(() => {
    ...
    if(typeof oldGerar === "function"){
      window.gerarWordEtiquetaFromForm = async function(){
        ...
        if(ok && payload){ try{ localStorage.setItem(LAST_LABEL_KEY, JSON.stringify(payload)); }catch(e){} addLog("info", "etiqueta", `Etiqueta gerada: ${payload.codigoEtiqueta || payload.serie || "sem cÃƒÂ³digo"}`); }
        return ok;
      };
    }
  }, 800);
  ...
  async function reimprimirUltimaEtiquetaWord(){
    try{
      ...
      if(!payload) return typeof mostrarMensagem === "function" && mostrarMensagem("Ainda nÃƒÂ£o existe uma etiqueta para reimprimir.", "erro");
      ...
      addLog("info", "etiqueta", "ReimpressÃƒÂ£o da ÃƒÂºltima etiqueta");
    } catch(e){ addLog("error", "etiqueta", e.message || e); if(typeof mostrarMensagem === "function") mostrarMensagem("Erro ao reimprimir a ÃƒÂºltima etiqueta.", "erro"); }
  }
...
(function(){
  ...
  function setupSidebarGroups(){
    ...
    if (sidebar.dataset.sidebarGroupsReady !== "1") {
      ...
      /* Guarda o estado real que o utilizador deixou antes de mudar de pÃ¡gina. */
    ...
    /* Se ainda nÃ£o existia estado guardado, grava o layout inicial desta pÃ¡gina para as prÃ³ximas. */
...
(function(){
  ...
  const PAGES = [
    ...
    { href:"add-toner.html", label:"Adicionar Toner", icon:"âž•" },
    { href:"stock.html", label:"Stock", icon:"ðŸ“¦" },
    { href:"historico.html", label:"HistÃ³rico", icon:"ðŸ§¾" },
    { href:"tarefas.html", label:"Tarefas", icon:"âœ…" },
    { href:"scanner-ia.html", label:"Scanner IA", icon:"ðŸ“„" },
    { href:"etiquetas-word.html", label:"Etiquetas Word", icon:"ðŸ·ï¸" },
    { href:"impressoras.html", label:"Impressoras", icon:"ðŸ–¨ï¸" },
    { href:"manutencao-impressoras.html", label:"ManutenÃ§Ã£o Impressoras", icon:"ðŸ› ï¸" },
    { href:"computadores.html", label:"Computadores", icon:"ðŸ’»" },
    { href:"pistolas.html", label:"Pistolas CK65", icon:"ðŸ“Ÿ" },
    { href:"radios.html", label:"RÃ¡dios", icon:"ðŸ“¡" },
    { href:"portas.html", label:"Portas Rede", icon:"ðŸ”Œ" },
    { href:"diretorio.html", label:"DiretÃ³rio", icon:"â˜Žï¸" },
    { href:"informacoes.html", label:"InformaÃ§Ãµes", icon:"â„¹ï¸" },
    { href:"users.html", label:"Users", icon:"ðŸ‘¥" },
    { href:"diagnostico.html", label:"DiagnÃ³stico", icon:"ðŸ©º" },
    { href:"config.html", label:"ConfiguraÃ§Ãµes", icon:"âš™ï¸" }
  ];
  ...
  function renderFavorites(){
    ...
    section.innerHTML = `
      <div class="sidebar-section-title">
        <span><span>â­</span><strong> Favoritos</strong></span>
        <button class="sidebar-fav-edit" type="button" title="Editar favoritos" aria-label="Editar favoritos">âœ¦</button>
      </div>
      <div class="sidebar-fav-list"></div>
    `;
    const list = section.querySelector(".sidebar-fav-list");
    if(!pages.length){
      list.innerHTML = '<div class="empty-favs">Sem favoritos. Carrega em âœ¦ para escolher.</div>';
    }else{
    ...
    overlay.innerHTML = `
      <div class="sidebar-fav-manage-card" role="dialog" aria-modal="true" aria-label="Editar favoritos">
        <div class="sidebar-fav-manage-head">
          <div><h3>Editar favoritos</h3><p>Escolhe as pÃ¡ginas que queres sempre no topo da sidebar.</p></div>
          <button class="sidebar-fav-close" type="button" aria-label="Fechar">Ã—</button>
        </div>
        ...
        <div class="sidebar-fav-actions">
          <button class="secondary-btn" type="button" data-fav-reset>Repor padrÃ£o</button>
...
/* ===== APP BRAGA v1.35.6 - NOTIFICAÃ‡Ã•ES: ESTADO, HISTÃ“RICO E REPARAÃ‡ÃƒO ===== */
...
async function obterPushSubscriptionAtualApp() {
  try {
    ...
    console.warn("DiagnÃ³stico push: sem subscription atual", error);
    return null;
  }
}
...
async function atualizarDiagnosticoPushAutomaticoApp() {
  ...
  if (window.electronAPI?.showNotification && !endpoint) {
    setCloudNotificationDiagnosticApp("Electron nÃ£o cria Web Push real neste ambiente. Para PC, usa a PWA instalada pelo Edge/Chrome.", "warn");
  } else if (endpoint) {
    setCloudNotificationDiagnosticApp("Este dispositivo tem endpoint Web Push. Se o teste falhar, o problema estÃ¡ na Cloud Function/Firestore.", "ok");
  }
...
function renderCloudDevicesNotificacoesApp(items = []) {
  ...
  setCloudNotificationTextApp("cloudDevicesStatus", `${activeItems.length} registados`, activeItems.length ? "ok" : "warn");
  setCloudNotificationTextApp("cloudDevicesDetail", `${webPushItems.length} Web Push Â· ${remoteItems.length} remotos prontos`);

  if (!activeItems.length) {
    host.innerHTML = `<div class="empty-state mini">Ainda nÃ£o hÃ¡ dispositivos registados.</div>`;
    return;
  }
  ...
  host.innerHTML = `
    <table class="notification-devices-pro-table">
      <thead>
        <tr><th>Dispositivo</th><th>Tipo</th><th>Push</th><th>Ãšltimo contacto</th><th>Estado</th></tr>
      </thead>
      <tbody>
        ${activeItems.map((item) => {
          ...
          return `
            <tr class="${isCurrent ? "is-current" : ""}">
              <td data-label="Dispositivo"><div class="notification-device-main"><strong>${escapeHtmlAppBraga(device)}${isCurrent ? " Â· Este" : ""}</strong><small>${escapeHtmlAppBraga(endpoint)}</small></div></td>
              ...
              <td data-label="Ãšltimo contacto">${escapeHtmlAppBraga(updated)}</td>
...
async function repararTodosRegistosCloudNotificacoesApp() {
  try {
    ...
    setCloudNotificationDiagnosticApp(`ReparaÃ§Ã£o concluÃ­da. ${disabled} registo(s) antigo(s) desativado(s).`, "ok");
...
async function carregarHistoricoNotificacoesCloudApp(showMessage = false) {
  ...
  try {
    ...
    if (!items.length) {
      host.innerHTML = `<div class="empty-state mini">Ainda nÃ£o hÃ¡ histÃ³rico de envios cloud.</div>`;
      return;
    }
    host.innerHTML = items.map((item) => `
      <div class="notification-history-item">
        <div>
          <strong>${escapeHtmlAppBraga(item.title || item.event || "NotificaÃ§Ã£o")}</strong>
          <small>${escapeHtmlAppBraga(item.body || "")} Â· ${escapeHtmlAppBraga(formatTimestampApp(item.createdAt || item.lastRunAt))}</small>
    ...
    if (showMessage) mostrarMensagem("HistÃ³rico atualizado.", "sucesso");
  } catch (error) {
    console.warn("HistÃ³rico notificationHistory indisponÃ­vel, a tentar auditLogs", error);
    try {
      ...
      host.innerHTML = items.length ? items.map((item) => `
        <div class="notification-history-item">
          <div><strong>${escapeHtmlAppBraga(item.title || item.event || "NotificaÃ§Ã£o")}</strong><small>${escapeHtmlAppBraga(formatTimestampApp(item.createdAt))}</small></div>
        ...
        </div>`).join("") : `<div class="empty-state mini">Sem histÃ³rico disponÃ­vel.</div>`;
    } catch (innerError) {
      host.innerHTML = `<div class="empty-state mini">Erro ao carregar histÃ³rico.</div>`;
    }
  }
}
...
(function(){
  ...
  const PAGE_ICONS = {
    "index.html":"ðŸ ", "add-toner.html":"âž•", "stock.html":"ðŸ“¦", "historico.html":"ðŸ§¾",
    "tarefas.html":"âœ…", "scanner-ia.html":"ðŸ“„", "etiquetas-word.html":"ðŸ·ï¸",
    "impressoras.html":"ðŸ–¨ï¸", "manutencao-impressoras.html":"ðŸ› ï¸", "computadores.html":"ðŸ’»",
    "pistolas.html":"ðŸ“Ÿ", "radios.html":"ðŸ“¡", "portas.html":"ðŸ”Œ", "diretorio.html":"â˜Žï¸",
    "informacoes.html":"â„¹ï¸", "users.html":"ðŸ‘¥", "diagnostico.html":"ðŸ©º",
    "config.html":"âš™ï¸"
  };
...
/* ===== APP BRAGA v1.54.1 - SIDEBAR NOVA GLOBAL ===== */
(function(){
  ...
  function getSidebars(){
    return Array.from(document.querySelectorAll("aside.sidebar, .sidebar-pro-groups, #sidebar, .enterprise-sidebar"));
  }
  ...
  function applyCollapsed(value, persist){
    ...
    document.documentElement.classList.toggle("sidebar-collapsed", active);
    document.body.classList.toggle("sidebar-collapsed", active);
    document.querySelectorAll(".app-sidebar-v4-collapse").forEach((button) => {
      button.textContent = active ? ">" : "<";
      button.setAttribute("aria-label", active ? "Expandir sidebar" : "Recolher sidebar");
  ...
  function render(){
    ...
    sidebar.removeAttribute("data-app-sidebar-v4-extra");
...
/* ===== END APP BRAGA v1.54.1 - SIDEBAR NOVA GLOBAL ===== */
