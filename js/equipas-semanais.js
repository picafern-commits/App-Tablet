(function(){
  ...
  function fmtDateTime(ms){if(!ms)return 'â€”';try{return new Intl.DateTimeFormat('pt-PT',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(ms));}catch{return 'â€”';}}
  ...
  function weekUntilLabel(start){const today=mondayOf(new Date());const diff=Math.round((mondayOf(start)-today)/(7*24*60*60*1000));if(diff===0)return 'Esta semana';if(diff===1)return 'PrÃ³xima semana';if(diff>1)return `Daqui a ${diff} semanas`;return `${Math.abs(diff)} semana(s) atrÃ¡s`;}
  ...
  async function addHistory(action,teamName){const payload={action,teamName:teamName||'',createdAtMs:Date.now(),user:(window.currentUser?.nome||window.currentUser?.email||'Sistema')};history.unshift({idDoc:`local-h-${Date.now()}`,...payload});history=history.slice(0,50);saveLocal();try{if(db()?.collection)await db().collection(HISTORY_COLLECTION).add(payload);}catch(e){console.warn('HistÃ³rico local:',e);}render();}
  ...
  function renderTable(){
    ...
    if(!pageRows.length){body.innerHTML='<tr><td colspan="8"><div class="eq-empty">Sem equipas para mostrar.</div></td></tr>';}
      ...
      return `<tr>
        <td><div class="eq-week-cell"><span class="eq-star">${isCurrent?'â˜…':'â˜†'}</span><span>Semana ${weekNumber(r.weekStart)} ${isCurrent?'<span class="eq-current-badge">ATUAL</span>':''}</span></div></td>
        <td><div class="eq-period">${periodLabel(r.weekStart)}</div></td>
        <td><div class="eq-team-name">${escapeHtml(team.nome||'Equipa sem nome')}</div><div class="eq-team-sub">â— ${members.length} membros</div></td>
        <td><div class="eq-avatars">${renderAvatars(members)}</div></td>
        <td><div class="eq-resp"><span class="eq-resp-avatar">${escapeHtml(initials(team.responsavel||members[0]?.nome||'R'))}</span>${escapeHtml(team.responsavel||members[0]?.nome||'Sem responsÃ¡vel')}</div></td>
        <td><span class="eq-badge ${escapeHtml(r.situacao)}">${r.situacao==='ativa'?'Ativa':r.situacao==='agendada'?'Agendada':'Planeada'}</span></td>
        <td>${created?fmtDateTime(created):'â€”'}</td>
        <td><div class="eq-actions"><button class="eq-icon-btn" data-action="edit" data-id="${escapeHtml(team.idDoc)}" title="Editar">âœŽ</button><button class="eq-icon-btn" data-action="detail" data-week="${escapeHtml(weekKey(r.weekStart))}" data-id="${escapeHtml(team.idDoc)}" title="Ver membros">ðŸ‘¥</button><button class="eq-icon-btn danger" data-action="delete" data-id="${escapeHtml(team.idDoc)}" title="Apagar">â‹®</button></div></td>
      </tr>`;
    }).join('');
  ...
  function renderPagination(total){const box=$('eqPagination');if(!box)return;const pages=Math.max(1,Math.ceil(total/pageSize));if(page>pages)page=pages;let html=`<button ${page<=1?'disabled':''} data-page="${page-1}">Â«</button>`;for(let i=1;i<=pages;i++){if(i<=5||i===pages||Math.abs(i-page)<=1){html+=`<button class="${i===page?'active':''}" data-page="${i}">${i}</button>`;}else if(i===6&&pages>7){html+=`<button disabled>â€¦</button>`;}}html+=`<button ${page>=pages?'disabled':''} data-page="${page+1}">Â»</button>`;box.innerHTML=html;}
  function renderSide(){
  ...
  function renderSelected(){const count=$('eqMembersCount'),box=$('eqSelectedMembers');if(count)count.textContent=`${selectedMembers.length} membros`;if(!box)return;if(!selectedMembers.length){box.textContent='Ainda nÃ£o escolheste nenhum membro.';return;}box.innerHTML=selectedMembers.map(m=>`<span class="eq-chip">${escapeHtml(m.nome||m.name||memberKey(m))}<button type="button" data-remove-member="${escapeHtml(m.id||m.idDoc||m.nome)}">Ã—</button></span>`).join('');}
  function addMember(){const id=$('eqUserSelect')?.value;if(!id)return;const user=users.find(u=>userId(u)===id);if(!user)return;if(selectedMembers.some(m=>String(m.id)===id))return toast('Esse user jÃ¡ estÃ¡ na equipa.','erro');selectedMembers.push({id,nome:userName(user),email:user.email||user.email_bragalis||''});$('eqUserSelect').value='';renderSelected();}
