...
async async function apagarPistola(ref) {
  ...
  try {
    ...
    try {
      const item = itemPorRef ? itemPorRef(pistolasData, ref) : null;
...
// ===== CK65 FIREBASE SAFE CRUD =====

async function sincronizarPistolasFirebaseSafe() {
  try {

    if (!window.db || !db.collection) return;

    db.collection("pistolas_ck65")
      .onSnapshot((snapshot) => {

        if (!Array.isArray(pistolasData)) return;
        ...
        if (typeof prepararRefsPistolas === "function") {
          prepararRefsPistolas();
        }
    ...
    console.error("CK65 Firebase:", e);
  }
}

setTimeout(() => {
  try {
    sincronizarPistolasFirebaseSafe();
  } catch(e){}
}, 1500);
