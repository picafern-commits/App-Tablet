...
function renderTonerDiagnostics() {
  ...
  logEl.innerHTML = tonerDiagnosticsState.log.map(item => `
    <div class="diagnostics-log-item">
      ...
      <strong><a href="http://<a href="http://<a href="http://${item.ip}" target="_blank" style="color:#60a5fa;text-decoration:none;font-weight:700;">${item.ip}</a>" target="_blank" style="color:#60a5fa;text-decoration:none;font-weight:700;"><a href="http://${item.ip}" target="_blank" style="color:#60a5fa;text-decoration:none;font-weight:700;">${item.ip}</a></a>" target="_blank" style="color:#60a5fa;text-decoration:underline;">🌐 <a href="http://<a href="http://${item.ip}" target="_blank" style="color:#60a5fa;text-decoration:none;font-weight:700;">${item.ip}</a>" target="_blank" style="color:#60a5fa;text-decoration:none;font-weight:700;"><a href="http://${item.ip}" target="_blank" style="color:#60a5fa;text-decoration:none;font-weight:700;">${item.ip}</a></a></a></strong> · ${item.message}
    </div>
  `).join("");
}
...
async function atualizarTonerKyocera(ip, index){

  try{

    const toner =
      await window.electronAPI.obterToner(ip);

    if(!toner) return;

    if(window.impressoras?.[index]){

      window.impressoras[index].black =
        toner.black;

      window.impressoras[index].toner =
        toner.black;
