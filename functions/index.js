...
const KNOCKOUT_MANUAL_VAULT_DOC_V355 = "settings/knockoutManual";
...
exports.notifyKnockoutManualVaultUpdatedV355 = onDocumentWritten(KNOCKOUT_MANUAL_VAULT_DOC_V355, async event => {
  const before = event.data?.before?.data() || {};
  const after = event.data?.after?.data() || {};
  if (!after.knockout || knockoutSignal(before) === knockoutSignal(after)) return;

  const title = "Fase final atualizada";
  const body = "A Fase Final manual foi guardada no cofre protegido.";
  const tokens = await loadEnabledTokens({ pref: "knockout", excludeUid: after.updatedBy || "" });
  ...
  const sent = await sendTokenNotifications(tokens, () => ({
    ...
    data: {
      type: "knockout",
      id: "settings-knockoutManual",
      uid: cleanString(after.updatedBy),
      title,
      body,
      tag: "mundial-knockout-manual-vault-updated",
      url: notificationUrl("knockout", "knockout")
    }
  }));
  logger.info("Notificacoes do cofre manual da fase final enviadas", { sent });
});
