...
async function broadcast(title, body, data) {
  ...
  await updateCollectionDoc("config", "cloudNotifications", {
    provider: "electron-watcher",
    region: "pc-local",
    lastTitle: title,
    lastBody: body,
    ...
    lastSent: sent,
    lastFailed: failed,
    lastDeviceCount: tokens.length,
    lastFcmTargets: fcmTargets,
    lastStandardWebPushTargets: standardWebPushTargets,
    standardWebPushReady: !!(webpush && VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY),
    lastRunAt: Date.now(),
    updatedAt: Date.now()
  }).catch((error) => console.warn(`Nao foi possivel atualizar estado cloudNotifications: ${error.message}`));
  return {
    sent,
    failed,
    standardWebPushReady: !!(webpush && VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY)
...
async function handleNotificationRequestPush(doc, config) {
  ...
  if (fields.status === "processing" || fields.status === "sent") return;
  ...
  await updateCollectionDoc("notificationRequests", id, {
    status: "processing",
    processingAt: Date.now(),
    processedBy: "electron-watcher"
  });
  try {
